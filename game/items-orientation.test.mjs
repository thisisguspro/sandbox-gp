...
// ---------- wave internals: foam crowns the wall, crest leans into travel ----------
{
  ...
  // crest lean: top of the wall should sit FURTHER along +X (forward) than its base
  const wpos = wall.geometry.getAttribute("position");
  let topX = 0, topN = 0, botX = 0, botN = 0;
  for (let i = 0; i < wpos.count; i++) {
    const v = new THREE.Vector3().fromBufferAttribute(wpos, i).applyEuler(wall.rotation);
    if (v.y > 0.6) { topX += v.x; topN++; } else if (v.y < -0.6) { botX += v.x; botN++; }
  }
  const lean = topX / Math.max(1, topN) - botX / Math.max(1, botN);
  (lean > 0.15) ? ok(`wave crest leans into travel (+${lean.toFixed(2)} local X)`) : no(`crest lean ${lean.toFixed(2)} — tips sideways/backward`);
}
