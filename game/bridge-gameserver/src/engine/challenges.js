  ...
  for (let s = 40; s < track.total - 20; s += gap) {
    ...
    for (const frac of [-0.36, 0, 0.36]) {   // wider than the grab radius: the choice is REAL
      ...
      boxes.push({
        ...
        y: p.y || 0,   // ON the road it belongs to — 66 of 159 boxes were at sea level under/over elevated track
        x: p.x + (-p.tz) * lat,
        z: p.z + (p.tx) * lat,
        sample: best,
        active: true,
...
function buildRings(track, player, rng) {
  ...
  for (let g = 0; g < 6; g++) {
    ...
    gates.push({
      ...
      z: p.z + (p.tx) * lat,
      y: p.y || 0,
      r: 1.7,   // smaller hoops: you AIM at them now — mid-road autopilot threads nothing
      hit: false,
    });
  }
...
// ARENA ITEM BOXES
//
// The challenge minigames (rings/ribbon/keys) string themselves along the racing
// LINE — an arena has no line, so they can't apply. But arena modes still want
// the "grab a box, get something to throw" loop the race has. So arena boxes are
// simpler: scattered across the disc, and touching one hands you a tier-weighted
// item DIRECTLY (no gauntlet). Same pickup feel, no racing line required.
// ============================================================================
import { insideWall } from "./shared/arenas.js";

const ARENA_BOX_RADIUS = 2.0;
const ARENA_BOX_WAVE_SEC = 12;   // grabbed boxes come back on this cadence

export function makeArenaBoxes(arena, rng = Math.random) {
  if (!arena) return [];
  const boxes = [];
  const count = Math.max(8, Math.round(arena.radius / 6));   // scale to the bowl
  let id = 0, guard = 0;
  while (boxes.length < count && guard++ < count * 20) {
    const a = rng() * Math.PI * 2;
    const r = Math.sqrt(rng()) * arena.radius * 0.82;         // uniform over disc
    const x = Math.cos(a) * r, z = Math.sin(a) * r;
    if (insideWall(arena, x, z, 2.5)) continue;               // never inside cover
    // keep them off the exact centre of CTF/Artist objectives
    boxes.push({ id: `abox${id++}`, x, z, active: true, respawnAt: 0 });
  }
  return boxes;
}

// Touch → a held item, tier-weighted mid-low (same distribution bots get on the
// track). Boxes go dormant and return together on a short wave so the field
// never runs dry. Returns events to surface.
export function tickArenaBoxes(engine, dt) {
  const boxes = engine.itemBoxes;
  if (!boxes || !boxes.length) return [];
  const out = [];
  if (engine._arenaBoxWaveAt == null) engine._arenaBoxWaveAt = engine.now + ARENA_BOX_WAVE_SEC;
  if (engine.now >= engine._arenaBoxWaveAt) {
    engine._arenaBoxWaveAt = engine.now + ARENA_BOX_WAVE_SEC;
    for (const b of boxes) b.active = true;
  }
  for (const box of boxes) {
    if (!box.active) continue;
    for (const p of engine.players.values()) {
      if (p.finished || p.eliminated || p.spectating || p.crumbledUntil) continue;
      if (p.heldItem) continue;                                // one at a time
      if (Math.hypot(p.x - box.x, p.z - box.z) > ARENA_BOX_RADIUS + CAR.BODY_RADIUS * 0.6) continue;
      box.active = false;
      const r = engine.rng();
      const tier = r > 0.9 ? "s" : r > 0.68 ? "gold" : r > 0.34 ? "silver" : "bronze";
      const itemId = rollItemTiered(engine.rng, tier);
      if (itemId === "kite") { applySelfKite(engine, p); }     // the dud latches on
      else p.heldItem = { id: itemId, tier };
      p.mChallenges = (p.mChallenges || 0) + 1;
      out.push({ type: "arena_pickup", playerId: p.id, itemId, tier, x: box.x, z: box.z });
      break;
    }
  }
  return out;
