...
export function buildTrack(def) {
  ...
  // BASE DECK HEIGHT: some tracks don't have hills — they have ALTITUDE. The
  // pier is a dock: the ENTIRE lap rides one flat deck above the water, and
  // everything downstream (grid poses, item boxes, minigame markers, physics
  // ground) reads sample.y, so lifting the samples lifts the whole world of
  // the map in one move. Uniform, so slopes and lap logic are untouched.
  if (def.baseY) {
    for (const smp of samples) smp.y = (smp.y || 0) + def.baseY;
  }
...
export const TRACKS = {
  sandcastle: {
    ...
    hazardSpec: [
      { kind: "oil",       frac: 0.14, lat: 0.55, r: 4 },    // wet sand on the T1 exit
...
TRACKS.shingle = {
    hazardSpec: [
      { kind: "oil",  frac: 0.22, lat: 0.0, r: 2.4 },          // wet stone after the tide — narrowed: a line past it must always exist
      { kind: "oil",  frac: 0.66, lat: 0.5, r: 4 },
...
TRACKS.pier = {
    ...
    hazardSpec: [
      { kind: "wave", frac: 0.12, lat: 0.0, r: 2.0, force: 4.5, side: 1 },
      { kind: "wave", frac: 0.38, lat: 0.0, r: 2.0, force: 4.5, side: -1 },
      { kind: "wave", frac: 0.62, lat: 0.0, r: 2.0, force: 5.5, side: 1 },
      { kind: "wave", frac: 0.86, lat: 0.0, r: 2.0, force: 5.5, side: -1 },
    ],
  id: "pier",
  ...
  baseY: 3.2,             // <- the dock: the whole lap rides the deck, up on its pilings
  points: [
...
TRACKS.volcano = {
  ...
  hazardSpec: [
    ...
    { kind: "lava", frac: 0.20, lat: 0.0, r: 2.6 },
    { kind: "lava", frac: 0.55, lat: 0.4, r: 4 },
    { kind: "lava", frac: 0.80, lat: -0.4, r: 4 },
    // ASH VENTS: they blind you, and you're doing 90.
    { kind: "ash", frac: 0.35, lat: 0.0, r: 2.7 },
    { kind: "ash", frac: 0.68, lat: 0.0, r: 2.7 },
