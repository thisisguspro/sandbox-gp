...
export function stepCar(state, input, dt, track, mods = {}) {
  ...
  let ground = 0;
  if (onRibbon && !smp.gap) {
    // (guard: unit tests hand in a mock track with no sample array)
    const nxt = track.samples ? track.samples[(i + 1) % track.samples.length] : null;
    if (nxt && !nxt.gap) {
      // how far between sample i and i+1 are we? Project onto the segment.
      const sx = nxt.x - smp.x, sz = nxt.z - smp.z;
      const segLen2 = sx * sx + sz * sz;
      let f = 0;
      if (segLen2 > 1e-6) {
        f = ((state.x - smp.x) * sx + (state.z - smp.z) * sz) / segLen2;
        f = Math.max(0, Math.min(1, f));
      }
      ground = (smp.y || 0) * (1 - f) + (nxt.y || 0) * f;
    } else {
      ground = smp.y || 0;
    }
  }
