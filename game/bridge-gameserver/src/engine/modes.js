...
export const DERBY = {
  ...
  view(e, p) {
    if (e.mode_ring == null) return null;    // lobby
    return { lives: p.lives ?? 0, ring: Math.round(e.mode_ring) };
  },
  worldView(e) {
    // The lobby builds a view BEFORE init() has ever run, so none of this state
    // exists yet. Returning null there is correct — there is no arena to describe
    // until the match starts. Throwing (which is what this used to do) killed the
    // whole broadcast, and the client never learned the mode had changed.
    if (!e.mode_wreckers) return null;
    return {
...
export const CTF = {
    ...
    for (const flag of e.mode_flags) {
      ...
      if (flag.carrier) {
        ...
        if (!c || c.eliminated || c.crumbledUntil) {
          ...
          c.modeSpeedMult = 0.92;
          e.erodePlayer?.(c, 0.10 * dt, null);
          // scored?
          const home = a.bases[c.team];
          const ownFlag = e.mode_flags[c.team];
          // THE HEAVY FLAG: nobody carries forever. Past 45s the flag slips —
          // it drops at your feet. This is the liveness valve that makes the
          // classic rule below safe: the measured both-teams-hold-forever
          // deadlock now self-resolves, because a held flag always comes loose.
          if (flag.carriedSince && e.now - flag.carriedSince > 45) {
            flag.carrier = null;
            flag.droppedAt = e.now;
            flag.carriedSince = 0;
            e._events.push({ type: "flag_dropped", team: flag.team, x: flag.x, z: flag.z, heavy: true });
            continue;
          }
          if (dist(c, home) < 6) {
            // THE CLASSIC RULE, restored: your own flag must be HOME to score.
            // The historical deadlock (both teams holding, forever) is broken by
            // the 45-second heavy-flag drop above plus touch-returns — a held
            // flag always comes loose, so the lock always opens.
            const ownHome = !ownFlag.carrier &&
              Math.hypot(ownFlag.x - ownFlag.homeX, ownFlag.z - ownFlag.homeZ) < 1;
            if (!ownHome) {
              if (!c._blockedSaid || e.now - c._blockedSaid > 6) {
                c._blockedSaid = e.now;
                e._events.push({ type: "flag_blocked", playerId: c.id, team: c.team });
              }
              continue;
            }
  ...
  view(e, p) {
    if (!e.mode_flags) return null;          // lobby
    ...
    // and the direction to the OBJECTIVE, for the on-screen indicator. The
    // objective flips depending on what you're doing:
    //   • carrying the enemy flag → point HOME (your own base), so "get it home"
    //     has an arrow that actually points home. (This was the bug: it kept
    //     pointing at the enemy flag you were already holding.)
    //   • otherwise → point at the enemy flag you're trying to grab.
    const a = arena;
    const enemyFlag = e.mode_flags[1 - p.team];
    const carrying = e.mode_flags.some((f) => f.carrier === p.id);
    const home = a.bases[p.team];
    const target = carrying ? { x: home.x, z: home.z } : { x: enemyFlag.x, z: enemyFlag.z };
    // Bearing RELATIVE TO HEADING. atan2(dz,dx) is the world angle to the target;
    // subtract the kart heading so 0 = dead ahead, which is what the HUD arrow
    // rotates by.
    const ang = Math.atan2(target.z - p.z, target.x - p.x) - p.heading;
    return {
      team: p.team,
      ...
      flagDist: Math.round(Math.hypot(target.x - p.x, target.z - p.z)),
      objective: carrying ? "home" : "enemyFlag",
    };
  },

  worldView(e) {
    if (!e.mode_flags) return null;      // lobby: init() hasn't run yet
    return {
...
export const ARTIST = {
  ...
  // Two extra minutes to draw: a round is now 195s (was 75). Drawing a
  // recognisable picture by driving a kart is slow, so the drawer needs the time.
  // Overall match cap (safety net; the match really ends when every player has
  // had their drawing round). Cover 8 drawers × 195s with headroom so the B1
  // draw-timer never guillotines a legitimate game mid-round.
  durationSec: 8 * 195 + 120,
  roundSec: 195,
  guessSec: 5,                  // stand in a hallway this long to lock it in

  // bot goals computed in tick() land here for the engine's arena driver
  botTarget(e, p) {
    return p._botGoal || null;
  },
  ...
  tick(e, dt) {
    ...
    // ---- bot brains ----
    // The drawer bot PAINTS: water on, driving a slow circle inside the canvas
    // (a crude ring, but unmistakably "someone drawing"). Guesser bots pick a
    // hall once per round — mostly wrong early, increasingly right as the
    // drawing develops — then drive there and park, like a person would.
    if (drawer?.isBot && !drawer.eliminated) {
      drawer._painting = true;
      const t = (e.now - (e.mode_roundStart || e.now)) * 0.35;
      const rr = Math.max(4, (a.canvas?.r || 12) * 0.55);
      drawer._botGoal = {
        x: (a.canvas?.x || 0) + Math.cos(t) * rr,
        z: (a.canvas?.z || 0) + Math.sin(t) * rr,
      };
    }
    for (const p of e.players.values()) {
      if (!p.isBot || p.id === e.mode_drawer || p.spectating || p.eliminated) continue;
      if (p._artRound !== e.mode_roundIdx) {
        p._artRound = e.mode_roundIdx;
        const elapsed = e.now - (e.mode_roundStart || e.now);
        const pCorrect = Math.min(0.85, 0.2 + 0.65 * (elapsed / (ARTIST.roundSec || 195)));
        const correctIdx = (e.mode_options || []).indexOf(e.mode_word);
        let pick = Math.floor(e.rng() * (a.halls?.length || 1));
        if (correctIdx >= 0 && e.rng() < pCorrect) pick = correctIdx;
        p._artHall = pick;
        // think a while before committing — instant answers read as random
        p._artGoAt = e.now + 6 + e.rng() * 18;
      }
      if (e.now >= (p._artGoAt || 0) && a.halls?.[p._artHall]) {
        const h = a.halls[p._artHall];
        p._botGoal = { x: h.x, z: h.z };
      } else {
        p._botGoal = null;
      }
    }
    ...
    // TOGGLE, not hold. Tap the paint key (Space) once and water starts pouring
    // from the back of the kart wherever you drive; tap again to stop. (Holding
    // Space down the whole time was awkward and, if the keydown was ever missed,
    // felt like "paint only works once".) Only inside the canvas, only the drawer.
    if (drawer && !drawer.eliminated) {
      const keyDown = !!drawer.input?.keys?.PAINT;
      // rising edge → flip the pouring state
      if (keyDown && !drawer._paintKeyWas) {
        drawer._painting = !drawer._painting;
      }
      drawer._paintKeyWas = keyDown;
      const painting = !!drawer._painting;
      ...
      if (painting && inCanvas) {
        ...
        // with paint ON would spawn thousands of coincident points
  ...
  view(e, p) {
    if (!e.mode_options) return null;        // lobby
    ...
    return {
      ...
      // is the paint currently flowing? (toggle state) — HUD shows PAINTING / tap
      // to draw so the drawer knows it's on.
      painting: isDrawer ? !!(e.players.get(e.mode_drawer)?._painting) : false,
  ...
  worldView(e) {
    if (!e.mode_strokes) return null;    // lobby: init() hasn't run yet
    return {
...
function startRound(e) {
  ...
  for (const p of e.players.values()) {
    ...
    p.erosion = 0;
    p._painting = false;        // paint toggle resets each round
    p._paintKeyWas = false;
  }
...
export const TAG = {
  ...
  view(e, p) {
    if (!e.mode_it) return null;             // lobby
    return {
      it: e.mode_it,
      amIt: p.id === e.mode_it,
...
export const PEARL = {
  ...
  view(e, p) {
    if (!e.mode_pearls) return null;         // lobby
    return {
      pearls: p.pearls ?? 0,
  ...
  worldView(e) {
    if (!e.mode_pearls) return null;     // lobby: init() hasn't run yet
    return {
      leader: e.mode_leader,
...
// Roster rule: every mode is 4 min / 8 max, so a mode you filled to 8 can never
// silently start 8-strong after you switch to one that "only holds 4" — they all
// hold the same range now. The ONE exception is Time Attack, which is a solo
// speed check: you, the track, the clock. (Bot-fill tops any lobby up to 4.)
export const MODE_LIST = [
  { id: "race", label: "Grand Prix", blurb: "Laps. Items. The classic.", min: 4, max: 8, arena: null },
  { id: "timeattack", label: "Time Attack", blurb: "Solo speed check. You, the track, the clock. Ranked.", min: 1, max: 1, arena: null },
  { id: "derby", label: "Demolition Derby", blurb: "Last kart standing. The wreckers hunt you too.", min: 4, max: 8, arena: "pit" },
  { id: "ctf", label: "Capture the Flag", blurb: "Teams. Walls block sight. Steal it and run.", min: 4, max: 8, arena: "fort" },
  { id: "artist", label: "Sand Artist", blurb: "Draw with water. Drive to the word you believe.", min: 4, max: 8, arena: "gallery" },
  { id: "tag", label: "Riptide Tag", blurb: "One kart is IT. Don't be holding it at the horn.", min: 4, max: 8, arena: "pit" },
  { id: "pearl", label: "Pearl Rush", blurb: "Grab the most pearls. Get hit, drop them.", min: 4, max: 8, arena: "lagoon" },
];
