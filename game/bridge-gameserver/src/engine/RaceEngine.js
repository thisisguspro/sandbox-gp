    ...
    for (const p of list) {
      ...
      if (this.arena) {
        ...
        for (const hz of this.arena.hazards || []) {
          ...
          if (hz.kind === "sink") {
            // quicksand: hard speed cap + steady erosion — sit in it and crumble
            const cap = CAR.MAX_SPEED * 0.4;
            if (p.speed > cap) p.speed = Math.max(cap, p.speed - 30 * dt);
            this.erodePlayer?.(p, 0.22 * dt, null);
          } else if (hz.kind === "deep") {
            // deep water: it takes you FAST. crumble → respawn → (pearl mode)
            // your pearls scatter right where you went under.
            this.erodePlayer?.(p, 0.9 * dt, null);
            p.speed *= 0.96;
          } else if (hz.kind === "lava") {
            this.erodePlayer?.(p, 0.55 * dt, null);
          }
          break;
        }
