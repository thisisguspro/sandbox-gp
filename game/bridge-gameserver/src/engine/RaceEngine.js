...
export class RaceEngine {
  ...
  addPlayer(name, account = null) {
    ...
    this.players.set(id, {
      ...
      lapTimes: [], _lapStartT: null,   // per-lap split times for the results table
  ...
  start({ force = false } = {}) {
    ...
    // the lap clock for EVERYONE starts the moment the flag drops
    this._raceStartT = this.now;
    for (const p of this.players.values()) p._lapStartT = this.now;
  ...
  // ONE WHEEL PER HIT. Hazards no longer sandblast you continuously — any
  // hazard contact costs exactly 1 of your 3, then you're immune for 1.6s.
  // Park in deep water and the clock keeps hitting you; clip an oil slick at
  // speed and it's one wheel, clean and readable.
  hazardHit(p) {
    if (this.now < (p._hazImmuneUntil || 0)) return;
    p._hazImmuneUntil = this.now + 2.0;
    this.erodePlayer(p, 1.0, null);
    this._events.push({ type: "hazard_hit", playerId: p.id });
  }
  ...
  requestReset(playerId) {
    ...
    // the shovel needs a STOPPED kart: no resetting out of a spin or an oil
    // slide at speed — stop first, then scoop
    if (Math.abs(p.speed) > 0.6 || (p.spinUntil && this.now < p.spinUntil)) {
      this._events.push({ type: "reset_blocked", playerId: p.id });
      return;
    }
    ...
    if (this.arena) {
      ...
      p._hazImmuneUntil = this.now + 2.0;   // fresh spawn, 2s untouchable
  ...
  tick(dt) {
    ...
    for (const p of list) {
      ...
      if (p.crumbledUntil) {
        ...
        p.crumbledUntil = 0;
        p._hazImmuneUntil = Math.max(p._hazImmuneUntil || 0, this.now + 2.0);   // re-molded: 2s grace
      ...
      if (this.arena) {
        ...
        // ---- arena hazards actually DO something now ----
        // They were painted on the floor and fully ignored by the sim: the
        // lagoon's deep water never fished anyone out, the fort's sinkholes
        // never sank anyone. Pearl Rush had no deaths, so pearls never spilled.
        for (const hz of this.arena.hazards || []) {
          if (Math.hypot(p.x - hz.x, p.z - hz.z) > hz.r) continue;
          if (hz.kind === "sink") {
            const cap = CAR.MAX_SPEED * 0.4;
            if (p.speed > cap) p.speed = Math.max(cap, p.speed - 30 * dt);
            this.hazardHit(p);
          } else if (hz.kind === "deep") {
            // linger and the 1.6s clock keeps hitting you — loiterers still drown
            this.hazardHit(p);
            p.speed *= 0.96;
      ...
      // every OTHER track hazard (oil, waves, ash) now costs a wheel too
      if (p.hazContact && !p.lavaBurn) this.hazardHit(p);
      // FALL DAMAGE: drop off the overpass and the landing costs a wheel —
      // the shortcut still exists, it just isn't free
      if (p.hardLanding > 7) this.hazardHit(p);
  ...
  _trackProgress(p) {
    ...
    if (p.nextCp < cps.length) {
      ...
      if (this._crossed(prevS, cur, 0, lapLen)) {
        p.lap++;
        // record the split: results shows a per-lap breakdown per racer
        const t0 = p._lapStartT ?? this._raceStartT ?? this.now;
        p.lapTimes.push(+(this.now - t0).toFixed(3));
        p._lapStartT = this.now;
        p.nextCp = 0;
  ...
  viewFor(playerId) {
    ...
    const players = [...this.players.values()].map((p) => ({
      ...
      team: p.team ?? null,
      imm: Math.max(0, Math.round(((p._hazImmuneUntil || 0) - this.now) * 1000)),
