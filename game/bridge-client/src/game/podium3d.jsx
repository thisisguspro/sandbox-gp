...
export default function Podium3D({ top3 = [], width = 460, height = 250 }) {
  ...
  useEffect(() => {
    ...
    // The steps' face plates are built on the +Z side. The camera is at +Z. If the
    // whole assembly reads as "the back of the podium", the fix is to turn the
    // assembly — not to keep arguing with the geometry.
    ...
    // Shiny plastic, like everything else — Lambert can't catch light, so the podium
    // read as flat cardboard next to karts that now have a proper sheen.
    const mat = (c) => new THREE.MeshPhongMaterial({ color: c, specular: 0x6a6a6a, shininess: 42 });
    ...
    for (const s of stepDefs) {
      ...
      // ---- THE FACE PLATE ----
      //
      // This was a dark navy slab (0x0b3140) from the old dark theme, and it sat
      // there blank. A podium step's front is where the PLACE NUMBER goes — it's the
      // whole reason the step has a front. Without it you're looking at three
      // anonymous boxes and you can't tell which is which.
      // The face plate goes on BOTH sides. I have now got the podium's facing wrong
      // twice by reasoning about it instead of looking at it — so build the number
      // on both faces and the entire class of bug disappears. A real podium has its
      // numbers on the front, and "the front" is simply whichever side you're on.
      const NUM = {
        1: [[0, 0.34], [0, 0.0], [0, -0.34], [-0.16, 0.22]],
        2: [[-0.14, 0.34], [0.14, 0.16], [0, -0.02], [-0.14, -0.2], [0.06, -0.34]],
        3: [[-0.10, 0.34], [0.14, 0.16], [0, 0], [0.14, -0.16], [-0.10, -0.34]],
      }[s.place] || [];

      for (const zSide of [1, -1]) {
        const face = new THREE.Mesh(new THREE.BoxGeometry(1.9, s.h, 0.04), mat(0xfff7ea));
        face.position.set(s.x, s.h / 2, zSide * 0.97);
        world.add(face);

        for (const [dx, dy] of NUM) {
          const bit = new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.16, 0.03), mat(s.color));
          // mirror the number on the far side, so it reads correctly from both
          bit.position.set(s.x + dx * zSide, s.h / 2 + dy, zSide * 1.01);
          world.add(bit);
        }
      }

      // a bright lip along the top edge of the step
      const lip = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.06, 2.0), mat(0xfff7ea));
      lip.position.set(s.x, s.h + 0.03, 0);
      world.add(lip);
    }
    // ground disc
    ...
    for (const s of stepDefs) {
      ...
      // ---- THE PODIUM WAS SHOWING ITS BACK ----
      //
      // I reasoned about this twice and got it wrong twice, because I cannot see the
      // screen and Gustavo can. He says it's turned; it's turned.
      //
      // The camera sits out at +Z. A kart's forward is +X (that's what heading=0
      // means, and the chassis is rotated at build time to match). To point the nose
      // at the camera you rotate +X onto +Z — and the sign of that rotation is the
      // thing I kept flipping. This is the one that puts the NOSE, the driver's face
      // and the numbered step faces toward the viewer, with a slight three-quarter
      // turn so it reads as a hero shot rather than a mugshot.
