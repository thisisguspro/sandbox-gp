...
export function buildWorld(scene, track) {
  ...
  const sunDeg = P.sun || { elevation: 14, azimuth: 225 };
  ...
  U.turbidity.value = P.haze ?? 5;
  U.rayleigh.value = sunDeg.elevation < 12 ? 2.6 : 1.6;   // low sun = warmer scatter
  U.mieCoefficient.value = 0.006;
  U.mieDirectionalG.value = 0.85;
  U.sunPosition.value.copy(sunDir);
  scene.add(sky);
  scene.userData.sky = sky;
  scene.userData.exposure = P.exposure ?? 1.12;
  ...
  const sun = new THREE.DirectionalLight(sunCol, sunDeg.elevation < 12 ? 2.5 : 2.0);
  ...
  const ground = new THREE.Mesh(
    ...
    isWater
      ? new THREE.MeshStandardMaterial({ color: P.water, roughness: 0.06, metalness: 0, envMapIntensity: 1.35, transparent: true, opacity: 0.96 })
    ...
    // rendering error.
    {
      ...
      for (let i = 0; i < n2; i += 14) {
        ...
        for (const s of [-1, 1]) {
          ...
          if (nearLowerRoad(track, px, pz, ay, 2.0)) continue;  // never in the underpass lane
      ...
      // ---- THE TUNNEL ----
      // Where the deck passes OVER a lower road, the lower road should run through
      // an enclosed underpass: solid side walls (no more glitching through a
      // half-there wall) and a ceiling that is the deck's own soffit. Walls are
      // built along the LOWER road's edges, tall enough to meet the deck, so you
      // drive through a tunnel instead of under a floating slab dotted with poles.
      const tunnelWall = plastic(P.stoneDark || 0xa8834f, { side: THREE.DoubleSide });
      const seen = new Set();
      for (let j = 0; j < n2; j += 2) {
        const q = track.samples[j];
        if (q.gap) continue;
        const qy = q.y || 0;
        // is a higher deck overhead here?
        let deckY = null;
        for (let i = 0; i < n2; i += 2) {
          const d = track.samples[i];
          if (d.gap) continue;
          if ((d.y || 0) - qy < 2.2) continue;
          if (Math.hypot(d.x - q.x, d.z - q.z) < track.width / 2 + 2.5) { deckY = d.y || 0; break; }
        }
        if (deckY == null) continue;
        seen.add(j);
        const qn = track.samples[(j + 2) % n2];
        for (const s of [1, -1]) {
          const wx0 = q.x + (-q.tz * s) * (track.width / 2 + 0.3);
          const wz0 = q.z + (q.tx * s) * (track.width / 2 + 0.3);
          const wx1 = qn.x + (-qn.tz * s) * (track.width / 2 + 0.3);
          const wz1 = qn.z + (qn.tx * s) * (track.width / 2 + 0.3);
          const len = Math.hypot(wx1 - wx0, wz1 - wz0) || 1;
          const wallH = (deckY - 0.55) - qy;
          if (wallH < 1) continue;
          const wall = new THREE.Mesh(new THREE.PlaneGeometry(len, wallH), tunnelWall);
          wall.position.set((wx0 + wx1) / 2, qy + wallH / 2, (wz0 + wz1) / 2);
          wall.rotation.y = -Math.atan2(wz1 - wz0, wx1 - wx0) + Math.PI / 2;
          scene.add(wall);
          // guide lights down the tunnel: cyan on the left of travel, pink on
          // the right — the same colour grammar as the edge trim, so even in
          // the dark of an underpass you know which way is forward
          if ((j % 6) === 0) {
            const dot = new THREE.Mesh(
              new THREE.SphereGeometry(0.09, 6, 5),
              neon(s > 0 ? P.neonCyan : P.neonPink, { opacity: 0.95 })
            );
            dot.position.set((wx0 + wx1) / 2 - (-q.tz * s) * 0.3, qy + 2.35, (wz0 + wz1) / 2 - (q.tx * s) * 0.3);
            scene.add(dot);
          }
        }
      }
      ...
      // ---- PORTAL MOUTHS ----
      // The tunnel needs to read as CUT, not as a slot you fell into: a framed
      // opening at each end — two jambs, a hazard-striped lintel capped with a
      // neon tube — sitting just outside the covered run, square across the
      // lower road, with full kart headroom under the lintel.
      {
        const idx = [...seen].sort((x, y) => x - y);
        const runs = [];
        if (idx.length) {
          let run = [idx[0]];
          for (let k = 1; k < idx.length; k++) {
            if (idx[k] - idx[k - 1] <= 4) run.push(idx[k]);
            else { runs.push(run); run = [idx[k]]; }
          }
          runs.push(run);
          if (runs.length > 1) {
            const first = runs[0], last = runs[runs.length - 1];
            if (first[0] + n2 - last[last.length - 1] <= 4) { runs[0] = last.concat(first); runs.pop(); }
          }
        }
        const stripeTex = (() => {
        ...
        const mkPortal = (jRaw) => {
          ...
          for (let i = 0; i < n2; i += 2) {
            ...
            if (d.gap) continue;
            if ((d.y || 0) - qy < 2.2) continue;
            if (Math.hypot(d.x - q.x, d.z - q.z) < track.width / 2 + 3.5) { deckY = d.y || 0; break; }
          }
          const H = Math.max(3.6, Math.min(qy + 4.4, deckY - 0.7) - qy);
          const grp = new THREE.Group();
          const jambMat = plastic(P.stoneDark || 0x6e5a44);
          for (const s of [1, -1]) {
            const jamb = new THREE.Mesh(new THREE.BoxGeometry(0.9, H, 0.9), jambMat);
            jamb.position.set(s * (track.width / 2 + 0.75), H / 2, 0);
            grp.add(jamb);
          }
          const lintel = new THREE.Mesh(
            new THREE.BoxGeometry(track.width + 3.0, 0.7, 1.0),
            new THREE.MeshStandardMaterial({ map: stripeTex, roughness: 0.8 })
          );
          lintel.position.y = H + 0.35;
          grp.add(lintel);
          const tube = new THREE.Mesh(new THREE.BoxGeometry(track.width + 3.0, 0.09, 0.09), neon(P.neonCyan, { opacity: 0.95 }));
          tube.position.y = H + 0.78;
          grp.add(tube);
...
function buildPilings(track) {
  ...
  for (let i = 0; i < track.samples.length; i += 8) {
    ...
    for (const s of [1, -1]) {
      ...
      if (!pier && nearLowerRoad(track, px, pz, h, 2.0)) continue;
    ...
    if ((i / 8) % 2 === 0 && h > 3 && !(!pier && nearLowerRoad(track, p.x, p.z, h, 2.6))) {
