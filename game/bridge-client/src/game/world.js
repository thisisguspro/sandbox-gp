...
export function buildWorld(scene, track) {
  ...
  const hemi = new THREE.HemisphereLight(P.ambient, P.sandLight, 0.55);   // painterly: lots of soft fill
  scene.add(hemi);
  ...
  const rim = new THREE.DirectionalLight(0xaac6e8, 0.26);
  ...
  const sun = new THREE.DirectionalLight(sunCol, sunDeg.elevation < 12 ? 1.45 : 1.35);  // softer key: shading flattens toward toon
  ...
  if (isWater) {
    ground = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x2f9fe8, roughness: 0.85, metalness: 0, envMapIntensity: 0.35 })   // flat saturated party-blue water   // glossy, not a MIRROR: the 0.06 sheet bounced the low sun straight into the camera as a white wall
    );
  ...
  // the normals average and the whole thing shades as one continuous surface.
  {
    ...
    // OVER THE CROSSING THE ROAD IS A BRIDGE, NOT A DUNE. Wherever this road
    // passes above the trestle zone, the sand skirt is skipped entirely — the
    // deck's own soffit + fascia (already built for every raised segment) plus
    // the suspension towers and cables are what you see. Sand aprons only wrap
    // the road out on the real hillsides.
    const bridged = new Array(n).fill(false);
    for (let i = 0; i < n; i++) {
      const p = track.at(i);
      if (p.gap) continue;
      if ((p.y || 0) >= 0.6 && overAnyLowerLane(track, p.x, p.z, p.y || 0, 26)) bridged[i] = true;
    }
    ...
    for (const side of [1, -1]) {
      ...
      const stride = ROWS + 1;
      for (let i = 0; i < n; i++) {
        // never bridge the jump — and no sand apron where the road IS a bridge
        if (track.at(i % n).gap || track.at((i + 1) % n).gap) continue;
        if (bridged[i % n] || bridged[(i + 1) % n]) continue;
        for (let r = 0; r < ROWS; r++) {
          const a = i * stride + r;
          const b = a + 1;
...
function buildBumpers(track) {
  ...
  for (let i = 0; i < track.samples.length; i += step) {
    ...
    // NO RAILS ON THE BRIDGE. Where the deck ramps up and over the lower
    // track, the noodles and their posts stood on nothing — floating beside a
    // suspended deck. The bridge's edge treatment is its fascia and cables;
    // the rails resume where the road is back on real ground.
    const aBr = (a.y || 0) >= 0.6 && overAnyLowerLane(track, a.x, a.z, a.y || 0, 26);
    const bBr = (b.y || 0) >= 0.6 && overAnyLowerLane(track, b.x, b.z, b.y || 0, 26);
    if (aBr || bBr) continue;
    for (const s of [1, -1]) {
