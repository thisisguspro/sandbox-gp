...
export function buildWorld(scene, track) {
    ...
    for (const side of [1, -1]) {
      ...
      for (let i = 0; i <= n; i++) {
        ...
        for (let r = 0; r <= ROWS; r++) {
          ...
          // ground instead of creasing at both ends. It no longer drops all the way
          // to the flat y=0 disc: on a high climb (Dunes hits +13) that left a cliff
          // out on the sand. Instead it settles to a gentle raised beach that keeps
          // a third of the road's height, so the ground reads as terrain that holds
          // the road up rather than a plateau hovering over a pit.
          const ease = f * f * (3 - 2 * f);
          const settle = (p.y || 0) * 0.33;        // where the far edge of the skirt rests
          const h = (p.y || 0) * (1 - ease) + settle * ease - 0.05;
  ...
  const groundAt = (x, z) => {
    ...
    const ease = f * f * (3 - 2 * f);
    const settle = (near.y || 0) * 0.33;
    return (near.y || 0) * (1 - ease) + settle * ease;
  };
  ...
  const placeProp = (group, x, z, yBase = 0) => {
    ...
    const ease = f * f * (3 - 2 * f);
    const settle = (near.y || 0) * 0.33;
    const groundY = (near.y || 0) * (1 - ease) + settle * ease - 0.05;
  ...
  // corners become recognisable because they have landmarks.
  {
    ...
    for (let i = 0; i < n; i += 5) {
      ...
      for (const side of [-1, 1]) {
        ...
        const settle = y * 0.33;
        const groundY = y * (1 - ease) + settle * ease - 0.05;
