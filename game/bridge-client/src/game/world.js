  ...
  // corners become recognisable because they have landmarks.
  {
    ...
    for (let i = 0; i < n; i += 5) {
      ...
      for (const side of [-1, 1]) {
        ...
        // STAND ON THE GROUND, not on the road. The old code planted everything at
        // the ROAD's height — so beside an 8.9m climb, a palm forty metres out hung
        // nine metres in the air. Those were the floating trees.
        const SKIRT_START = track.width / 2 + 1.2;
        const SKIRT_END = SKIRT_START + 34;
        const distOut = Math.hypot(x - p.x, z - p.z);
        let groundY = 0;
        if (y > 0.15 && distOut < SKIRT_END) {
          const f = Math.max(0, Math.min(1, (distOut - SKIRT_START) / (SKIRT_END - SKIRT_START)));
          groundY = y * (1 - f);
        }
