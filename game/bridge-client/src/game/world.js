...
export function buildWorld(scene, track) {
  ...
  // ground plane. Now there is always something holding the road up.
  {
    ...
    for (const side of [1, -1]) {
      for (let i = 0; i < n; i++) {
        const a = track.at(i % n);
        const b = track.at((i + 1) % n);
        if (a.gap || b.gap) continue;             // the bridge has nothing under it

        const ay = a.y || 0, by = b.y || 0;
        // skip flat ground — nothing to hold up, and it would z-fight the plane
        if (Math.abs(ay) < 0.15 && Math.abs(by) < 0.15) continue;

        const anx = -a.tz * side, anz = a.tx * side;
        const bnx = -b.tz * side, bnz = b.tx * side;
        const inner = track.width / 2 + 1.2;      // just outside the kerb

        // top edge: at the road's height, hugging the kerb
        const t0 = [a.x + anx * inner, ay - 0.05, a.z + anz * inner];
        const t1 = [b.x + bnx * inner, by - 0.05, b.z + bnz * inner];
        // bottom edge: out on the sand, at ground level
        const g0 = [a.x + anx * (inner + SKIRT), 0, a.z + anz * (inner + SKIRT)];
        const g1 = [b.x + bnx * (inner + SKIRT), 0, b.z + bnz * (inner + SKIRT)];

        // two triangles for the slope
        for (const [p, q, r] of [[t0, t1, g1], [t0, g1, g0]]) {
    ...
    if (pos.length) {
      ...
      eg.setAttribute("normal", new THREE.Float32BufferAttribute(nrm, 3));
      eg.computeVertexNormals();                  // real shading on the slopes
  ...
  // corners become recognisable because they have landmarks.
  {
    ...
    for (let i = 0; i < n; i += 5) {
      ...
      for (const side of [-1, 1]) {
        ...
        // Out beyond the shoulder. This was `width/2 + 9` — only nine metres past
        // the edge of the road, which is INSIDE the drivable shoulder. And because
        // a circuit loops back on itself, a palm placed nine metres off one straight
        // lands squarely in the middle of a DIFFERENT straight. Gustavo's screenshot
        // is full of tree trunks standing on the racing line.
        const off = (track.width / 2) + 16 + rnd() * 30;
        const x = p.x + (-p.tz) * off * side;
        const z = p.z + (p.tx) * off * side;
        const y = p.y || 0;

        // THE REAL GUARD: check this piece against EVERY sample of the whole track,
        // not just the one it was placed relative to. Nothing may sit within the
        // road plus its shoulder, anywhere on the circuit.
    ...
    for (let k = 0; k < 3; k++) {
      ...
      // the inward normal (pointing back at the road) is (+tz, -tx) for this side
      stand.rotation.y = -Math.atan2(-p.tx, p.tz);
      deco.add(stand);
    }
  }
