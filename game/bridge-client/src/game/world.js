...
export function buildWorld(scene, track) {
  ...
  U.rayleigh.value = sunDeg.elevation < 12 ? 1.5 : 1.6;   // low sun: paint the horizon, not the whole windshield
  U.mieCoefficient.value = 0.0035;   // shrink the sun's glow blob
  ...
  scene.userData.sky = sky;
  {
    ...
    const expBase = Math.min(P.exposure ?? 0.95, 0.92);
    scene.userData.exposure = (sunReq.elevation ?? 22) > 34 ? Math.min(expBase, 0.80)
                            : sunDeg.elevation < 12 ? Math.min(expBase, 0.82)
                            : expBase;
  }
  ...
  const ground = new THREE.Mesh(
    ...
    isWater
      ? new THREE.MeshStandardMaterial({ color: P.water, roughness: 0.18, metalness: 0, envMapIntensity: 0.95, transparent: true, opacity: 0.96 })   // glossy, not a MIRROR: the 0.06 sheet bounced the low sun straight into the camera as a white wall
    ...
    // rendering error.
    {
      ...
      for (let i = 0; i < n2; i += 14) {
        ...
        for (const s of [-1, 1]) {
          ...
          if (nearLowerRoad(track, px, pz, ay, 4.5)) continue;  // never in the underpass lane
          {
            // exposed-on-the-hillside check (see buildPilings): if the skirt
            // here is far below the deck, skip — the embankment reads as the
            // support, and a lone pillar top on the slope reads as a bug.
            let near = track.samples[0], nd = Infinity;
      ...
      const wallMat = plastic(P.stoneDark || 0x6e5a44, { emissive: 0x241a16, emissiveIntensity: 0.5 });
      const ceilMat = plastic(P.stone || 0x9a7a54, { emissive: 0x241a16, emissiveIntensity: 0.45, side: THREE.DoubleSide });
      for (const run of runs) {
        for (let k = 0; k < run.length; k++) {
          const j = run[k];
          const q = track.at(j), qn = track.at((j + 2) % n2);
          if (q.gap || qn.gap) continue;
          const qy = q.y || 0, qny = qn.y || 0;
          const cH0 = ceilAt(q), cH1 = ceilAt(qn);
          const cH = Math.min(cH0, cH1);
          // solid side walls, ground to ceiling
          for (const s of [1, -1]) {
            const off = track.width / 2 + 0.5;
            const wx0 = q.x + (-q.tz * s) * off, wz0 = q.z + (q.tx * s) * off;
            const wx1 = qn.x + (-qn.tz * s) * off, wz1 = qn.z + (qn.tx * s) * off;
            const len = Math.hypot(wx1 - wx0, wz1 - wz0) || 1;
            const wh = cH - Math.min(qy, qny) + 0.3;
            const wall = new THREE.Mesh(new THREE.BoxGeometry(len + 0.35, wh, 0.45), wallMat);
            wall.position.set((wx0 + wx1) / 2, Math.min(qy, qny) + wh / 2, (wz0 + wz1) / 2);
            wall.rotation.y = -Math.atan2(wz1 - wz0, wx1 - wx0);
            wall.castShadow = true;
            wall.receiveShadow = true;
            scene.add(wall);
            // guide lights: cyan left of travel, pink right — same grammar as
            // the edge trim, so the way forward is legible even in here
            if ((j % 6) === 0) {
              const dot = new THREE.Mesh(
                new THREE.SphereGeometry(0.10, 6, 5),
                neon(s > 0 ? P.neonCyan : P.neonPink, { opacity: 0.95 })
              );
              dot.position.set((wx0 + wx1) / 2 - (-q.tz * s) * 0.45, Math.min(qy, qny) + 2.3, (wz0 + wz1) / 2 - (q.tx * s) * 0.45);
              scene.add(dot);
            }
          }
          // the ceiling slab, wall to wall
          {
            const w2 = track.width / 2 + 0.95;
            const p0 = [q.x + (-q.tz) * w2, cH, q.z + (q.tx) * w2];
            const p1 = [qn.x + (-qn.tz) * w2, cH, qn.z + (qn.tx) * w2];
            const p2 = [qn.x - (-qn.tz) * w2, cH, qn.z - (qn.tx) * w2];
            const p3 = [q.x - (-q.tz) * w2, cH, q.z - (q.tx) * w2];
          ...
          // a warm lamp down the crown every other bay — the tunnel lights
          // itself instead of borrowing the blown-out daylight outside
          if ((j % 4) === 0) {
            const lamp = new THREE.Mesh(
              new THREE.SphereGeometry(0.13, 8, 6),
              new THREE.MeshBasicMaterial({ color: 0xffd9a0, transparent: true, opacity: 0.95, fog: false })
            );
            lamp.position.set((q.x + qn.x) / 2, cH - 0.22, (q.z + qn.z) / 2);
            scene.add(lamp);
            const halo = new THREE.Mesh(new THREE.SphereGeometry(0.34, 8, 6), neon(0xffc97a, { opacity: 0.18 }));
            halo.position.copy(lamp.position);
            scene.add(halo);
          }
        }
        // ---- THE OVERBURDEN ----
        // A tunnel that stands in the open is a shed. Bury it: a sand ridge
        // runs the length of each bore — outer wall base, up over the crown,
        // down the other side — tapering to nothing at the portals so the
        // mouths stay clear. It merges into the embankment hills beside it,
        // which is exactly the "cut the hill and connect it to the tunnel"
        // read: the hill IS the tunnel now.
        {
          const ridge = [];
          for (let k = 0; k < run.length; k++) {
            const j = run[k];
            const q = track.at(j), qn = track.at((j + 2) % n2);
            if (q.gap || qn.gap) continue;
            const cH = Math.min(ceilAt(q), ceilAt(qn));
            // taper the mound down toward both portals
            const edge = Math.min(k, run.length - 1 - k);
            const lift = edge === 0 ? 0.25 : edge === 1 ? 0.7 : 1.0;
            const crown = cH + 1.1 * lift;
            const baseOff = track.width / 2 + 1.6;
            for (const [pt, nx, nz] of [[q, -q.tz, q.tx], [qn, -qn.tz, qn.tx]]) {
              const py = pt.y || 0;
              ridge.push([
                [pt.x + nx * baseOff, py + 0.1, pt.z + nz * baseOff],
                [pt.x, crown, pt.z],
                [pt.x - nx * baseOff, py + 0.1, pt.z - nz * baseOff],
              ]);
            }
          ...
          for (let k = 0; k + 1 < ridge.length; k++) {
            const A = ridge[k], B = ridge[k + 1];
            for (let e = 0; e < 2; e++) {
              rv.push(...A[e], ...B[e], ...B[e + 1], ...A[e], ...B[e + 1], ...A[e + 1]);
            }
          }
          if (rv.length) {
            ...
            const mound = new THREE.Mesh(
              rg,
              new THREE.MeshStandardMaterial({ color: P.sandLight, roughness: 0.95, side: THREE.DoubleSide })
            );
      ...
      // ---- PORTAL MOUTHS ----
      // A framed opening at each end of every bore — jambs, a hazard-striped
      // lintel, a cyan neon tube over the top — so the entrance reads from a
      // straight away and the tunnel is something you SEE and aim for.
      {
        const stripeTex = (() => {
          const c = document.createElement("canvas");
          c.width = 128; c.height = 16;
          const gg = c.getContext("2d");
          for (let i = 0; i < 16; i++) { gg.fillStyle = i % 2 ? "#ffe14d" : "#181018"; gg.fillRect(i * 8, 0, 8, 16); }
          const t = new THREE.CanvasTexture(c);
          t.colorSpace = THREE.SRGBColorSpace;
          return t;
        })();
        const mkPortal = (jRaw) => {
          const j = ((jRaw % n2) + n2) % n2;
          const q = track.at(j);
          if (q.gap) return;
          const qy = q.y || 0;
          const H = ceilAt(q) - qy + 0.15;
          const grp = new THREE.Group();
          const jambMat = plastic(P.stoneDark || 0x6e5a44);
          for (const s of [1, -1]) {
            const jamb = new THREE.Mesh(new THREE.BoxGeometry(1.0, H, 1.0), jambMat);
            jamb.position.set(s * (track.width / 2 + 0.85), H / 2, 0);
            grp.add(jamb);
          }
          const lintel = new THREE.Mesh(
            new THREE.BoxGeometry(track.width + 3.4, 0.75, 1.1),
            new THREE.MeshStandardMaterial({ map: stripeTex, roughness: 0.8 })
          );
          lintel.position.y = H + 0.37;
          grp.add(lintel);
          const tube = new THREE.Mesh(new THREE.BoxGeometry(track.width + 3.4, 0.1, 0.1), neon(P.neonCyan, { opacity: 0.95 }));
          tube.position.y = H + 0.84;
          grp.add(tube);
          grp.position.set(q.x, qy, q.z);
          grp.rotation.y = Math.atan2(-q.tx, -q.tz);   // frame spans ACROSS the lower road
          scene.add(grp);
        };
        for (const run of runs) {
          mkPortal(run[0] - 2);
          mkPortal(run[run.length - 1] + 2);
        }
      }
    }
  }
  ...
  // A ring of dark towers with lit windows and neon slabs encircles every map,
  // out past the fog line, so wherever you look the wasteland ends in a
  // skyline. It hazes into the dusk correctly because it's real geometry.
  scene.add(buildSkylineRing(P));
  ...
  // corners become recognisable because they have landmarks.
  {
    ...
    const mkInstanced = (geo, mat, list, yOff = 0, tintCols = null, radius = 0) => {
    ...
    mkInstanced(new THREE.BoxGeometry(2.2, 0.08, 1.3), plastic(0xffffff), spots.towel, 0.05, TOY, 1.3);
    mkInstanced(new THREE.DodecahedronGeometry(1.0, 0), plastic(0xc0aa8a), spots.rock, 0.4, null, 1.4);
    mkInstanced(new THREE.SphereGeometry(0.8, 10, 8), plastic(0xfff7ea), spots.ball, 0.8, null, 0.8);
    mkInstanced(new THREE.CylinderGeometry(0.22, 0.3, 3.2, 6), plastic(0x9a8266), spots.wood, 0.3, null, 0.3);
...
function buildPilings(track) {
  ...
  for (let i = 0; i < track.samples.length; i += 8) {
    ...
    for (const s of [1, -1]) {
      ...
      if (!pier && nearLowerRoad(track, px, pz, h, 4.5)) continue;
      if (!pier) {
        // hillside height here (same smoothstep skirt the embankment uses): if
        // the ground is way below the post's top, the post would stand exposed
        // on the slope — those were the bare pillar tops on the climb.
        let near = track.samples[0], nd = Infinity;
        for (const q of track.samples) { const dd = Math.hypot(q.x - px, q.z - pz); if (dd < nd) { nd = dd; near = q; } }
        const SK0 = track.width / 2 - 0.7, SKL = 34;
        const f = Math.max(0, Math.min(1, (nd - SK0) / SKL));
