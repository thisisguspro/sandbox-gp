      ...
      for (const run of runs) {
        for (let k = 0; k < run.length; k++) {
          ...
          for (const s of [1, -1]) {
            ...
            qaReg("wall", (wx0 + wx1) / 2, (wz0 + wz1) / 2, 0, Math.min(qy, qny), cH);
      ...
      // straight away and the tunnel is something you SEE and aim for.
      {
        ...
        const mkPortal = (jRaw) => {
          ...
          for (const s of [1, -1]) {
            ...
            grp.add(jamb);
            const jx = s * (track.width / 2 + 0.85);
            qaReg("jamb", q.x - q.tz * jx, q.z + q.tx * jx, 0.55, qy, qy + H);
          }
          ...
          grp.add(tube);
          // the full arch outline: vertical tubes down the jamb faces + amber
          // marker lamps — the opening reads as a PORTAL from down the road
          for (const s of [1, -1]) {
            const vt = new THREE.Mesh(new THREE.BoxGeometry(0.1, H, 0.1), neon(P.neonCyan, { opacity: 0.95 }));
            vt.position.set(s * (track.width / 2 + 0.85), H / 2, 0.56);
            grp.add(vt);
            const lamp = new THREE.Mesh(new THREE.SphereGeometry(0.2, 8, 6),
              new THREE.MeshBasicMaterial({ color: 0xffc75a, fog: false }));
            lamp.position.set(s * (track.width / 2 + 1.7), 1.4, 0.4);
            grp.add(lamp);
          }
