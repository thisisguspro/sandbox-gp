...
export class Effects3D {
  ...
  _build(e) {
    ...
    if (e.kind === "balloon") {
      ...
      // ---- A SAND CONE ----
      // A crumbled kart leaves a clean CONE of sand, not a cluster of bubbly
      // spheres (which read as random blobs on the track). One cone, one flatter
      // skirt cone at the base so it sits into the ground, and a couple of kart
      // bits poking out so it still reads as "this WAS a car."
      const R = e.r || 2.3;

      // the cone itself
      const cone = new THREE.Mesh(new THREE.ConeGeometry(R * 0.92, R * 1.15, 20), this._sandMat);
      cone.position.y = R * 0.575;
      cone.castShadow = cone.receiveShadow = true;
      grp.add(cone);
      // a low, wider skirt cone so the base blends into the sand instead of
      // cutting a hard circle
      const skirt = new THREE.Mesh(new THREE.ConeGeometry(R * 1.12, R * 0.34, 20), this._sandMat);
      skirt.position.y = R * 0.17;
      skirt.receiveShadow = true;
      grp.add(skirt);

      // a SPADE stabbed into the side of the cone
      const handle = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 1.05, 6), plastic(0x2fa8d8));
      handle.position.set(R * 0.30, R * 0.72, R * 0.30);
      handle.rotation.z = -0.42;
      const blade = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.32, 0.05), plastic(0x2fa8d8));
      blade.position.set(R * 0.30 + 0.22, R * 0.72 - 0.47, R * 0.30);
      blade.rotation.z = -0.42;
      grp.add(handle, blade);

      // and a wheel from the kart, half-buried at the base — "this WAS a car"
      const wheel = new THREE.Mesh(new THREE.TorusGeometry(0.30, 0.12, 8, 12), plastic(PALETTE.tire));
      wheel.position.set(-R * 0.58, 0.30, -R * 0.34);
  ...
  syncStatuses(carRecs) {
    // The boost flames and the stun birds are ANIMATED, so this needs a clock.
    // It didn't have one — `t` was used in both and never defined, which threw a
    // ReferenceError every frame the moment anyone boosted or got stunned.
    const t = performance.now() / 1000;
    for (const [pid, rec] of carRecs) {
      ...
      // ---- TURBO: THE FULL ANIME TREATMENT ----
      //
      // The old streaks were five boxes at x = -1.6 — and the car's FORWARD is +X,
      // so they trailed off the SIDE of the kart instead of out the back. That's
      // Gustavo's "the flame of the boost is on the side of the car".
      //
      // And they were just... lines. A boost in an anime kart racer should be an
      // EVENT: twin exhaust flames with a white-hot core, a shockwave ring blowing
      // off the back, and speed lines raking past.
      if (info.turbo && !st.streaks) {
        const s = new THREE.Group();

        // TWIN EXHAUST FLAMES, out the BACK (-X, behind the car)
        for (const side of [-1, 1]) {
          // the outer flame — long, orange, translucent
          const flame = new THREE.Mesh(
            new THREE.ConeGeometry(0.28, 1.5, 8),
            plastic(0xff8c42, { transparent: true, opacity: 0.75 })
          );
          flame.rotation.z = Math.PI / 2;          // point it along -X
          flame.position.set(-1.55, 0.55, side * 0.30);
          flame.userData.flame = side;
          s.add(flame);

          // the WHITE-HOT CORE inside it
          const core = new THREE.Mesh(
            new THREE.ConeGeometry(0.13, 0.95, 6),
            plastic(0xfffdf0, { transparent: true, opacity: 0.95 })
          );
          core.rotation.z = Math.PI / 2;
          core.position.set(-1.30, 0.55, side * 0.30);
          core.userData.core = side;
          s.add(core);

          // a blue root where it leaves the pipe — that's what makes fire look HOT
          const root = new THREE.Mesh(
            new THREE.SphereGeometry(0.16, 8, 6),
            plastic(0x59b7e8, { transparent: true, opacity: 0.7 })
          );
          root.position.set(-1.05, 0.55, side * 0.30);
          s.add(root);
        }

        // THE SHOCKWAVE RING, blowing off the back
        const ring = new THREE.Mesh(
          new THREE.TorusGeometry(0.7, 0.06, 8, 20),
          plastic(0xd8fbff, { transparent: true, opacity: 0.5 })
        );
        ring.rotation.y = Math.PI / 2;
        ring.position.set(-1.7, 0.6, 0);
        ring.userData.shock = true;
        s.add(ring);

        // SPEED LINES raking past the kart — the anime staple
        for (let i = 0; i < 10; i++) {
          const line = new THREE.Mesh(
            new THREE.BoxGeometry(1.6 + Math.random() * 1.2, 0.035, 0.035),
            plastic(0xffffff, { transparent: true, opacity: 0.55 })
          );
          line.position.set(
            -1.0 - Math.random() * 2.2,
      ...
      // and ANIMATE it — a static flame is a sticker
      if (st.streaks) {
        const tt = t * 14;
        for (const c of st.streaks.children) {
          const u = c.userData;
          if (u.flame != null) {
            // the flame gutters
            c.scale.x = 1 + Math.sin(tt + u.flame) * 0.28;
            c.scale.y = 1 + Math.cos(tt * 1.3 + u.flame) * 0.14;
            c.scale.z = c.scale.y;
          } else if (u.core != null) {
            c.scale.x = 1 + Math.sin(tt * 1.7 + u.core) * 0.35;
          } else if (u.shock) {
            // the shockwave PULSES outward and fades
            const ph = (t * 2.2) % 1;
            c.scale.setScalar(0.5 + ph * 1.6);
            if (c.material) c.material.opacity = 0.55 * (1 - ph);
            c.position.x = -1.7 - ph * 1.1;
          } else if (u.speedline != null) {
            // the lines RAKE past, then loop
            const ph = ((t * 3.5 + u.speedline * 0.11) % 1);
            c.position.x = 0.4 - ph * 4.2;
            if (c.material) c.material.opacity = 0.6 * Math.sin(ph * Math.PI);
          }
        }
      }
      ...
      // ======================================================================
      // THE STUN: BIRDS AND STARS CIRCLING YOUR HEAD
      //
      // The oldest gag in animation, and it is IMMEDIATELY readable — you see it
      // and you know, without a word of text, that the kart in front of you is
      // helpless. That's exactly what a status effect is supposed to do.
      //
      // Three little birds orbiting on a tilted plane, chirping (see the audio),
      // with stars and tweety-symbols between them.
      // ======================================================================
      const isStunned = !!(info.stunned || info.crumbled || info.spinning);
      if (isStunned && !st.dazed) {
        const g = new THREE.Group();

        // THREE BIRDS, orbiting
        for (let i = 0; i < 3; i++) {
          const bird = new THREE.Group();

          const bBody = new THREE.Mesh(new THREE.SphereGeometry(0.10, 8, 6), plastic(0xfff7ea));
          bBody.scale.set(1.25, 0.9, 0.9);
          bird.add(bBody);

          const bHead = new THREE.Mesh(new THREE.SphereGeometry(0.065, 7, 6), plastic(0xfff7ea));
          bHead.position.set(0.10, 0.05, 0);
          bird.add(bHead);

          const beak = new THREE.Mesh(new THREE.ConeGeometry(0.028, 0.06, 4), plastic(0xf5a623));
          beak.position.set(0.17, 0.04, 0);
          beak.rotation.z = -Math.PI / 2;
          bird.add(beak);

          // an eye — a bird with no eye is a dumpling
          const bEye = new THREE.Mesh(new THREE.SphereGeometry(0.016, 5, 4), plastic(0x1c1712));
          bEye.position.set(0.13, 0.07, 0.045);
          bird.add(bEye);

          // WINGS — these flap
          for (const s of [-1, 1]) {
            const wing = new THREE.Mesh(new THREE.ConeGeometry(0.055, 0.16, 3), plastic(0xe8dcc0));
            wing.position.set(-0.01, 0.03, s * 0.08);
            wing.rotation.x = s * 1.2;
            wing.rotation.z = 0.4;
            wing.userData.wing = s;
            bird.add(wing);
          }
          // a tail
          const btail = new THREE.Mesh(new THREE.ConeGeometry(0.045, 0.10, 3), plastic(0xe8dcc0));
          btail.position.set(-0.13, 0.01, 0);
          btail.rotation.z = Math.PI / 2;
          bird.add(btail);

          bird.userData.bird = i;
          g.add(bird);
        }

        // STARS between them — the other half of the gag
        for (let i = 0; i < 5; i++) {
          const star = new THREE.Group();
          for (let k = 0; k < 5; k++) {
            const spike = new THREE.Mesh(new THREE.ConeGeometry(0.028, 0.11, 3), plastic(0xf7c04a));
            spike.rotation.z = (k / 5) * Math.PI * 2;
            spike.position.set(
              Math.sin((k / 5) * Math.PI * 2) * 0.05,
              Math.cos((k / 5) * Math.PI * 2) * 0.05,
              0
            );
            star.add(spike);
          }
          star.userData.star = i;
          g.add(star);
        }

        rec.mesh.add(g);
        st.dazed = g;
      } else if (!isStunned && st.dazed) {
        rec.mesh.remove(st.dazed);
        st.dazed = null;
      }

      // and MAKE THEM ORBIT
      if (st.dazed) {
        const HEAD_Y = 1.55;   // just above the driver's helmet
        for (const c of st.dazed.children) {
          const u = c.userData;
          if (u.bird != null) {
            const a = t * 2.6 + (u.bird / 3) * Math.PI * 2;
            c.position.set(
              Math.cos(a) * 0.55,
              HEAD_Y + Math.sin(a * 2) * 0.10,      // bobbing
              Math.sin(a) * 0.55
            );
            // face the way it's flying
            c.rotation.y = -a + Math.PI / 2;
            c.rotation.z = Math.sin(a * 2) * 0.2;
            // FLAP
            for (const w of c.children) {
              if (w.userData.wing) {
                w.rotation.x = w.userData.wing * (0.9 + Math.sin(t * 22 + u.bird) * 0.7);
              }
            }
          } else if (u.star != null) {
            const a = -t * 1.8 + (u.star / 5) * Math.PI * 2;   // orbit the OTHER way
            c.position.set(
              Math.cos(a) * 0.42,
              HEAD_Y + 0.18 + Math.cos(a * 3) * 0.06,
              Math.sin(a) * 0.42
            );
            c.rotation.z = t * 3 + u.star;
            const tw = 0.8 + Math.sin(t * 6 + u.star * 1.3) * 0.35;
            c.scale.setScalar(tw);
          }
        }
      }
    }
