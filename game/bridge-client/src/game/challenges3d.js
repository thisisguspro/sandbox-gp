  ...
  sync(boxes = []) {
    ...
    for (const b of boxes) {
      ...
      if (!rec) {
        ...
        const body = new THREE.Mesh(new THREE.BoxGeometry(1.5, 1.5, 1.5), plastic(KIND_COLOR[b.kind] || BOX_COLORS[i % BOX_COLORS.length]));
        ...
        const bi = track?.nearest ? track.nearest(b.x, b.z, -1, 0) : -1;
  ...
  sync(ch) {
    ...
    if (!this.group) {
      ...
      this.gates = ch.gates.map((g, i) => {
        ...
        grp.add(ring, post);
        const gi = track?.nearest ? track.nearest(g.x, g.z, -1, 0) : -1;
        const gy = gi >= 0 ? (track.at(gi).y || 0) : 0;
