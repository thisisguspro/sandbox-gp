  ...
  sync(boxes = []) {
    ...
    for (const b of boxes) {
      ...
      if (!rec) {
        ...
        // The hologram presentation is GONE by request: no octahedron skin, no
        // wireframe cage, no beam — the bucket alone is the pickup. It still
        // floats and spins (animate() drives the group), which reads clearly
        // as "grab me" without any line-art wrapped around it.
...
export class Ribbon3D {
  ...
  sync(ch, meState) {
    ...
    const t = this.track;
    let j = t.nearest(meState.x, meState.z, meState.sampleHint ?? -1, meState.y ?? 0);   // level-locked: never the other pass
