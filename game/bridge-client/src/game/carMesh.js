...
import { PALETTE, plastic , makeFaceTexture } from "./palette.js";

// ============================================================================
// THE MATERIAL
//
// EVERYTHING in this game was MeshLambertMaterial — flat, matte, no highlight, no
// sheen. Lambert has no specular term at all: it cannot catch the light. That is
// why every item, however many parts I bolted onto it, still read as "shapes
// slapped together" — they were shapes made of MATTE CARDBOARD.
//
// A toy is moulded plastic. It is SHINY. It has a hot little highlight where the
// sun hits it, and that highlight is what tells your eye "this is a solid, curved,
// real object" — more than any amount of extra geometry ever will.
//
// MeshPhongMaterial with a tuned specular gives every single object in the game a
// plastic sheen, for free, everywhere. It costs nothing (Phong is still a cheap
// forward-shaded material) and it is the single highest-leverage change available:
// one line, and the whole game stops looking like paper craft.
// ============================================================================
const toon = (color, opts = {}) => {
  const { transparent, opacity, side, emissive, emissiveIntensity, map, roughness = 0.5, metalness = 0.2, ...rest } = opts;
  // PBR for every kart part: painted metal and moulded plastic under the same
  // sky as the world. The clearcoat body shell comes from bodyPlastic().
...
// it said it was.
// ============================================================================
// WHEELS v2 — REAL RUBBER, REAL SHAPES.
//
// The old system was a tint table: every wheel was the same torus with a
// different rim colour, which is exactly what Gustavo called out — "not just
// the color and not just the rim but the rubber too". Each id now builds its
// OWN construction on the one-axle contract (axle = X, holder→spin→visuals),
// so they roll correctly and LOOK like different wheels at ten metres.
// Premium wheels carry animators (holder.userData.anim) — collected into
// car.userData.cosmeticAnims and run by animateCar.
// ============================================================================
function buildWheel(shoeId) {
  const g = new THREE.Group();
  const spin = new THREE.Group();
  const holder = new THREE.Group();
  spin.add(g); holder.add(spin);
  const tor = (r, tube, col, opts) => {
    const m = new THREE.Mesh(new THREE.TorusGeometry(r, tube, 10, 18), opts?.basic
      ? new THREE.MeshBasicMaterial({ color: col, transparent: true, opacity: opts.opacity ?? 0.85, blending: THREE.AdditiveBlending, depthWrite: false, fog: false })
      : toon(col));
    m.rotation.y = Math.PI / 2;
    return m;
  };
  const rimCyl = (r, w, col, metal) => {
    const m = new THREE.Mesh(new THREE.CylinderGeometry(r, r, w, 14),
      metal ? new THREE.MeshStandardMaterial({ color: col, metalness: 0.85, roughness: 0.25 }) : toon(col));
    m.rotation.z = Math.PI / 2;
    return m;
  };
  const lugRing = (n, col, size, radius) => {
    const grp2 = new THREE.Group();
    for (let k = 0; k < n; k++) {
      const a = (k / n) * Math.PI * 2;
      const lug = new THREE.Mesh(new THREE.BoxGeometry(size[0], size[1], size[2]), toon(col));
      lug.position.set(0, Math.cos(a) * radius, Math.sin(a) * radius);
      lug.rotation.x = a;
      grp2.add(lug);
    }
    return grp2;
  };
  const spokes = (n, col, metal) => {
    const grp2 = new THREE.Group();
    for (let k = 0; k < n; k++) {
      const sp = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.27, 0.05),
        metal ? new THREE.MeshStandardMaterial({ color: col, metalness: 0.85, roughness: 0.3 }) : toon(col));
      sp.rotation.x = (k / n) * Math.PI * 2;
      grp2.add(sp);
    }
    return grp2;
  };
  ...
  switch (shoeId) {
    case "shoes_sandal": {              // BEACH SLICKS — wide, low, sun-faded
      const t = tor(0.34, 0.20, 0x2c2620); t.scale.x = 1.25; g.add(t);
      g.add(tor(0.34, 0.035, 0x59b7e8)); // a single wave groove
      g.add(rimCyl(0.17, 0.30, 0xe8dcc0));
      break;
    }
    case "shoes_neon": {                // NEON RIMS — dark slick, live glow
      g.add(tor(0.34, 0.15, 0x14110e));
      const glow = tor(0.30, 0.045, 0xff5fa2, { basic: true });
      g.add(glow);
      g.add(rimCyl(0.17, 0.22, 0x241f2a));
      holder.userData.anim = (t) => { glow.material.opacity = 0.6 + Math.sin(t * 6) * 0.3; };
      break;
    }
    case "shoes_turbo": {               // TURBO RIMS — knurled lugs, warm metal
      g.add(tor(0.33, 0.145, 0x241f1a));
      g.add(lugRing(8, 0x3a3128, [0.20, 0.10, 0.09], 0.40));
      g.add(rimCyl(0.18, 0.23, 0xff8c42, true));
      const bolt = new THREE.Mesh(new THREE.SphereGeometry(0.09, 8, 6), toon(0xfff7ea));
      bolt.position.x = 0.13; g.add(bolt);
      break;
    }
    case "shoes_chrome": {              // CHROME — mirror spokes
      g.add(tor(0.34, 0.14, 0x1c1712));
      g.add(spokes(6, 0xd8dde3, true));
      g.add(rimCyl(0.12, 0.24, 0xd8dde3, true));
      break;
    }
    case "shoes_coral": {               // NACRE — pearl sidewall bumps
      g.add(tor(0.34, 0.155, 0x3a2a34));
      for (const sgn of [1, -1]) {
        for (let k = 0; k < 5; k++) {
          const a = (k / 5) * Math.PI * 2 + (sgn > 0 ? 0 : 0.6);
          const bump = new THREE.Mesh(new THREE.SphereGeometry(0.055, 7, 6), toon(0xffe8f0));
          bump.position.set(sgn * 0.12, Math.cos(a) * 0.30, Math.sin(a) * 0.30);
          bump.scale.x = 0.6;
          g.add(bump);
        }
      }
      g.add(rimCyl(0.16, 0.20, 0xfff1f6));
      break;
    }
    case "shoes_storm": {               // STORMTREADS — chunky knobbies + sparks
      g.add(tor(0.32, 0.13, 0x14171c));
      g.add(lugRing(10, 0x232a33, [0.22, 0.11, 0.10], 0.40));
      g.add(rimCyl(0.16, 0.24, 0x59b7e8, true));
      const s1 = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.05, 0.05),
        new THREE.MeshBasicMaterial({ color: 0xaee6ff, fog: false }));
      s1.position.set(0.16, 0.28, 0); g.add(s1);
      holder.userData.anim = (t) => { s1.visible = Math.sin(t * 17) > 0.55; };
      break;
    }
    case "shoes_comet": {               // COMET TREADS — the Mythic. Glow + tail.
      g.add(tor(0.34, 0.145, 0x1c1712));
      const glow = tor(0.36, 0.05, 0xffc83d, { basic: true, opacity: 0.9 });
      g.add(glow);
      g.add(rimCyl(0.17, 0.22, 0xffc83d, true));
      g.add(spokes(5, 0xfff1d6, true));
      // the comet tail hangs off the HOLDER (doesn't spin): three fading fins aft
      const tail = new THREE.Group();
      for (let k = 0; k < 3; k++) {
        const fin = new THREE.Mesh(new THREE.PlaneGeometry(0.5 - k * 0.12, 0.16 - k * 0.03),
          new THREE.MeshBasicMaterial({ color: 0xffc83d, transparent: true, opacity: 0.35 - k * 0.09, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide, fog: false }));
        fin.position.set(-0.45 - k * 0.30, 0.05, 0);
        tail.add(fin);
      }
      holder.add(tail);
      holder.userData.anim = (t) => {
        glow.material.opacity = 0.7 + Math.sin(t * 5) * 0.25;
        tail.children.forEach((f, k2) => { f.scale.y = 1 + Math.sin(t * 7 + k2) * 0.25; });
      };
      break;
    }
    case "shoes_wood": {                // BOARDWALK WAGONS — wooden cart wheels
      const drum = rimCyl(0.33, 0.20, 0x8a5f33); g.add(drum);
      for (let k = 0; k < 6; k++) {
        const slat = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.10, 0.66), toon(0xa9793f));
        slat.rotation.x = (k / 6) * Math.PI;
        g.add(slat);
      }
      g.add(tor(0.34, 0.045, 0x6b4a26));  // the rope hoop
      g.add(rimCyl(0.09, 0.24, 0x5a3d20));
      break;
    }
    default: {                          // SANDAL TREADS (starter) — flip-flop straps
      g.add(tor(0.34, 0.16, 0x3a2f24));
      g.add(lugRing(3, 0xf0c04a, [0.34, 0.09, 0.07], 0.34));
      g.add(rimCyl(0.19, 0.22, 0xf0c04a));
      g.add(spokes(5, 0xf0c04a));
      break;
    }
  }
  return holder;   // holder(y-steer + anim) → spin(rolls about X) → visuals
}
...
// ---- FLAGS ----
// The pennant streaming off the back of the kart. It's the thing every rival stares
// at for the entire race, so it's the most-seen cosmetic in the game.
const FLAG_COLORS = {
  flag_lifeguard: 0xe2574c,
  flag_surfpunk:  0x2fa8d8,
  flag_tiki:      0xff8c42,
  flag_neon:      0xff5fa2,
  flag_mecha:     0x9aa3ad,
  flag_coral:     0xffe8f0,
  flag_sandcastle:0xe8c98c,
  flag_storm:     0x4a5763,
  flag_golden:    0xf5a623,
  flag_checker:   0xfffdf0,
  flag_pirate:    0x1c1712,
};

// What's PRINTED on each flag. A flag with no device is a paint swatch — the
// emblem is the entire reason anybody wants one.
const FLAG_EMBLEMS = {
  // THE JOLLY ROGER — a skull and crossbones. The most wanted flag in any game.
  flag_pirate: () => {
    const g = new THREE.Group();
    const skull = M(new THREE.SphereGeometry(0.062, 10, 8), 0xfffdf0);
    skull.scale.set(1, 1.1, 0.35);
    skull.position.y = 0.025;
    g.add(skull);
    // the jaw
    const jaw = M(new THREE.BoxGeometry(0.075, 0.028, 0.03), 0xfffdf0);
    jaw.position.y = -0.048;
    g.add(jaw);
    // EYE SOCKETS — this is what makes it a skull and not an egg
    for (const s of [-1, 1]) {
      const socket = M(new THREE.SphereGeometry(0.022, 6, 5), 0x1c1712);
      socket.position.set(s * 0.026, 0.035, 0.022);
      socket.scale.z = 0.5;
      g.add(socket);
    }
    const nasal = M(new THREE.ConeGeometry(0.012, 0.024, 3), 0x1c1712);
    nasal.position.set(0, 0.004, 0.022);
    nasal.rotation.x = Math.PI;
    g.add(nasal);
    // teeth
    for (let i = 0; i < 3; i++) {
      const tooth = M(new THREE.BoxGeometry(0.014, 0.016, 0.02), 0x1c1712);
      tooth.position.set(-0.020 + i * 0.020, -0.048, 0.020);
      g.add(tooth);
    }
    // THE CROSSED BONES
    for (const s of [-1, 1]) {
      const bone = M(new THREE.CylinderGeometry(0.010, 0.010, 0.15, 5), 0xfffdf0);
      bone.position.set(0, -0.085, 0.012);
      bone.rotation.z = s * 0.8;
      g.add(bone);
      // the knuckle ends
      for (const e of [-1, 1]) {
        const knob = M(new THREE.SphereGeometry(0.017, 6, 5), 0xfffdf0);
        knob.position.set(
          Math.sin(s * 0.8) * e * 0.075,
          -0.085 - Math.cos(s * 0.8) * e * 0.075,
          0.012
        );
        g.add(knob);
      }
  ...
  // THE CHEQUERED FLAG — the finish line itself
  flag_checker: () => {
    const g = new THREE.Group();
    for (let r = 0; r < 4; r++) {
      for (let c = 0; c < 6; c++) {
        if ((r + c) % 2) continue;
        const sq = M(new THREE.BoxGeometry(0.075, 0.065, 0.006), 0x1c1712);
        sq.position.set(-0.19 + c * 0.076, 0.10 - r * 0.066, 0);
        g.add(sq);
      }
    }
    return g;
  ...
  // GOLDEN STANDARD — a sun disc
  flag_golden: () => {
    const g = new THREE.Group();
    const disc = M(new THREE.CylinderGeometry(0.075, 0.075, 0.008, 14), 0xfffdf0);
    disc.rotation.x = Math.PI / 2;
    g.add(disc);
    for (let i = 0; i < 8; i++) {
      const a = (i / 8) * Math.PI * 2;
      const ray = M(new THREE.ConeGeometry(0.016, 0.05, 3), 0xfffdf0);
      ray.position.set(Math.cos(a) * 0.10, Math.sin(a) * 0.10, 0);
  ...
  // STORM WARNING — a lightning bolt
  flag_storm: () => {
    const g = new THREE.Group();
    const segs = [[0.02, 0.09, 0.5], [-0.01, 0.02, -0.6], [0.02, -0.05, 0.5], [-0.01, -0.11, -0.5]];
    for (const [x, y, r] of segs) {
      const bolt = M(new THREE.BoxGeometry(0.028, 0.075, 0.006), 0xd8fbff);
  ...
  // COURT STANDARD — a shell
  flag_coral: () => {
    const g = new THREE.Group();
    const shell = M(new THREE.SphereGeometry(0.08, 10, 6, 0, Math.PI, 0, Math.PI / 2), 0xff5fa2);
    shell.rotation.x = -Math.PI / 2;
    shell.scale.z = 0.25;
    g.add(shell);
    for (let i = 0; i < 5; i++) {
      const rib = M(new THREE.BoxGeometry(0.008, 0.075, 0.006), 0xffe8f0);
      rib.position.set(-0.05 + i * 0.025, 0.02, 0.01);
      rib.rotation.z = (i - 2) * 0.22;
      g.add(rib);
    }
    return g;
  },

  // SQUADRON COLOURS — a mecha chevron
  flag_mecha: () => {
    const g = new THREE.Group();
    for (let i = 0; i < 3; i++) {
      for (const s of [-1, 1]) {
        const bar = M(new THREE.BoxGeometry(0.020, 0.085, 0.006), i % 2 ? 0xff8c42 : 0xd8dde3);
        bar.position.set(s * 0.055, 0.07 - i * 0.06, 0);
...
// wraps a shape with an animator: the closure lands in userData.anim and the
// collector wires it into car.userData.cosmeticAnims
function withAnim(g, makeAnim) { g.userData.anim = makeAnim(g); return g; }

// ---- NEW SHOWPIECES (the fresh line) ----
function propellerCap(c) {
  const g = new THREE.Group();
  const cap = new THREE.Mesh(new THREE.SphereGeometry(0.30, 12, 8, 0, Math.PI * 2, 0, Math.PI * 0.42), toon(c));
  cap.position.y = 0.55; g.add(cap);
  const stem = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.10, 6), toon(0xfff7ea));
  stem.position.y = 0.82; g.add(stem);
  const prop = new THREE.Group();
  for (const s of [1, -1]) {
    const blade = new THREE.Mesh(new THREE.BoxGeometry(0.34, 0.02, 0.09), toon(s > 0 ? 0xff5a4c : 0x2fb9c9));
    blade.position.x = s * 0.18; prop.add(blade);
  }
  prop.position.y = 0.88; g.add(prop);
  return withAnim(g, () => (t) => { prop.rotation.y = t * 14; });
}
function chefHat() {
  const g = new THREE.Group();
  const band = new THREE.Mesh(new THREE.CylinderGeometry(0.30, 0.30, 0.14, 12), toon(0xfff7ea));
  band.position.y = 0.62; g.add(band);
  for (let k = 0; k < 5; k++) {
    const a = (k / 5) * Math.PI * 2;
    const puff = new THREE.Mesh(new THREE.SphereGeometry(0.15, 9, 7), toon(0xffffff));
    puff.position.set(Math.cos(a) * 0.16, 0.82, Math.sin(a) * 0.16);
    g.add(puff);
  }
  const top = new THREE.Mesh(new THREE.SphereGeometry(0.17, 9, 7), toon(0xffffff));
  top.position.y = 0.90; g.add(top);
  return g;
}
function coneHat() {
  const g = new THREE.Group();
  const cone = new THREE.Mesh(new THREE.ConeGeometry(0.27, 0.55, 12), toon(0xff8c42));
  cone.position.y = 0.92; g.add(cone);
  const band = new THREE.Mesh(new THREE.CylinderGeometry(0.185, 0.215, 0.09, 12), toon(0xfff7ea));
  band.position.y = 0.90; g.add(band);
  const base = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.05, 0.5), toon(0xe07430));
  base.position.y = 0.66; g.add(base);
  return g;
}
function headphonesShape(c) {
  const g = new THREE.Group();
  const band = new THREE.Mesh(new THREE.TorusGeometry(0.31, 0.045, 8, 16, Math.PI), toon(0x241f2a));
  band.position.y = 0.52; g.add(band);
  for (const s of [1, -1]) {
    const cup = new THREE.Mesh(new THREE.CylinderGeometry(0.11, 0.11, 0.09, 10), toon(c));
    cup.rotation.z = Math.PI / 2;
    cup.position.set(s * 0.315, 0.50, 0);
    g.add(cup);
    const ring = new THREE.Mesh(new THREE.TorusGeometry(0.11, 0.02, 6, 12),
      new THREE.MeshBasicMaterial({ color: 0x2fe6c8, fog: false }));
    ring.rotation.y = Math.PI / 2;
    ring.position.set(s * 0.36, 0.50, 0);
    g.add(ring);
  }
  return withAnim(g, () => (t) => {
    g.children.forEach((m2, i2) => { if (m2.material?.color && m2.geometry?.type === "TorusGeometry" && m2.material.fog === false) m2.material.color.setHSL((t * 0.25 + i2 * 0.3) % 1, 0.8, 0.6); });
  });
}

// ---- HEADWEAR (attaches to the driver, sits on the helmet crown) ----
const HEAD_SHAPES = {
  head_propeller:  (c) => propellerCap(c),
  head_chef:       ()  => chefHat(),
  head_cone:       ()  => coneHat(),
  head_headphones: (c) => headphonesShape(c),
  ...
  head_marshal:  (c) => withAnim((() => {
    const g = crownShape(0x2fe6c8, true);
    const orbit = new THREE.Group();
    for (let k = 0; k < 3; k++) {
      const pearl = new THREE.Mesh(new THREE.SphereGeometry(0.05, 8, 6), new THREE.MeshBasicMaterial({ color: 0xeafffa, fog: false }));
      pearl.position.set(Math.cos((k / 3) * Math.PI * 2) * 0.42, 0, Math.sin((k / 3) * Math.PI * 2) * 0.42);
      orbit.add(pearl);
    }
    orbit.position.y = 0.86; g.add(orbit); g.userData.orbit = orbit;
    return g;
  })(), (g) => (t) => { g.userData.orbit.rotation.y = t * 1.6; }),   // MYTHIC: Crown of the Tides — orbiting pearls
  // wide-brim / caps
  ...
  head_halo:     (c) => withAnim(haloShape(0xfff1d6), (g) => (t) => { g.rotation.y = t * 0.9; g.position.y = Math.sin(t * 2.2) * 0.02; }),
  head_pineapple:(c) => pineappleHat(),
  head_flowercrown: (c) => withAnim(flowerCrown(), (g) => (t) => { g.rotation.y = t * 0.35; }),
  head_helmetwing: (c) => withAnim((() => {
    const g = wingedHelm(c);
    return g;
  })(), (g) => (t) => { g.rotation.z = Math.sin(t * 3.1) * 0.05; }),
  head_horns:    (c) => withAnim((() => {
    const g = hornsShape(c);
    for (const sn of [1, -1]) {
      const tip = new THREE.Mesh(new THREE.SphereGeometry(0.045, 7, 6), new THREE.MeshBasicMaterial({ color: 0xaee6ff, fog: false }));
      tip.position.set(sn * 0.22, 0.95, -0.02); g.add(tip); (g.userData.tips ||= []).push(tip);
    }
    return g;
  })(), (g) => (t) => { g.userData.tips.forEach((m2, i2) => { m2.visible = Math.sin(t * 11 + i2 * 2) > 0.2; }); }),
};
...
const SCARF_SHAPES = {
  ...
  bandana_trailblazer: (c) => withAnim(scarf(0xff5fa2, "long"), (g) => (t) => {
    g.traverse((m2) => { if (m2.material?.color) m2.material.color.setHSL((t * 0.12) % 1, 0.75, 0.62); });
  }),   // MYTHIC: Aurora Sash — the whole wrap cycles through the aurora
...
function jetpackFloat() {
  const g = new THREE.Group();
  for (const s of [1, -1]) {
    const can = new THREE.Mesh(new THREE.CapsuleGeometry(0.13, 0.30, 4, 8), toon(0x9aa3ad));
    can.position.set(-1.05, 0.9, s * 0.18);
    g.add(can);
    const nz = new THREE.Mesh(new THREE.ConeGeometry(0.10, 0.16, 8), toon(0x394047));
    nz.rotation.z = Math.PI / 2;
    nz.position.set(-1.28, 0.82, s * 0.18);
    g.add(nz);
    const flame = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.30, 7),
      new THREE.MeshBasicMaterial({ color: 0x7fd8ff, transparent: true, opacity: 0.85, blending: THREE.AdditiveBlending, depthWrite: false, fog: false }));
    flame.rotation.z = Math.PI / 2;
    flame.position.set(-1.45, 0.82, s * 0.18);
    g.add(flame);
    (g.userData.flames ||= []).push(flame);
  }
  g.userData.anim = (t) => { g.userData.flames.forEach((f, i2) => { f.scale.y = 0.7 + Math.sin(t * 21 + i2 * 1.7) * 0.4; f.material.opacity = 0.6 + Math.sin(t * 17 + i2) * 0.25; }); };
  return g;
}

const FLOATY_SHAPES = {
  tank_jetpack: () => jetpackFloat(),
...
const GEAR_SHAPES = {
  ...
  tool_kite:      (c) => kiteTool(null),
  kite_clown:     (c) => kiteTool("clown", 0xfff7ea),
  kite_shark:     (c) => kiteTool("shark", 0x59b7e8),
  kite_eyes:      (c) => kiteTool("eyes", 0xffd24d),
...
function makeKiteFace(kind) {
  const px = 128;
  const c = document.createElement("canvas"); c.width = c.height = px;
  const g = c.getContext("2d");
  g.clearRect(0, 0, px, px);
  const eye = (cx, cy, r) => { g.fillStyle = "#22201d"; g.beginPath(); g.ellipse(cx, cy, r, r * 1.25, 0, 0, Math.PI * 2); g.fill();
    g.fillStyle = "#fff"; g.beginPath(); g.ellipse(cx + r * 0.3, cy - r * 0.35, r * 0.3, r * 0.35, 0, 0, Math.PI * 2); g.fill(); };
  if (kind === "clown") {
    g.fillStyle = "#fff"; g.beginPath(); g.arc(64, 64, 52, 0, Math.PI * 2); g.fill();
    eye(46, 50, 9); eye(82, 50, 9);
    g.fillStyle = "#e2332a"; g.beginPath(); g.arc(64, 70, 12, 0, Math.PI * 2); g.fill();   // the nose
    g.strokeStyle = "#e2332a"; g.lineWidth = 7; g.lineCap = "round";
    g.beginPath(); g.arc(64, 74, 28, 0.15 * Math.PI, 0.85 * Math.PI); g.stroke();          // the grin
    g.fillStyle = "#2fa8d8";
    for (const [hx, hy] of [[18, 30], [110, 30]]) { g.beginPath(); g.arc(hx, hy, 13, 0, Math.PI * 2); g.fill(); }  // hair puffs
  } else if (kind === "shark") {
    g.fillStyle = "#8fa2ad"; g.beginPath(); g.arc(64, 64, 52, 0, Math.PI * 2); g.fill();
    eye(44, 46, 8); eye(84, 46, 8);
    g.fillStyle = "#3a2a2a"; g.beginPath(); g.ellipse(64, 84, 34, 20, 0, 0, Math.PI); g.fill();   // open mouth
    g.fillStyle = "#fff";
    for (let k = 0; k < 6; k++) { g.beginPath(); g.moveTo(36 + k * 11, 84); g.lineTo(41 + k * 11, 96); g.lineTo(47 + k * 11, 84); g.fill(); }  // teeth
  } else {   // "eyes" — just two huge peepers
    eye(42, 62, 17); eye(86, 62, 17);
  }
...
function kiteTool(face = null, cloth = 0xff5fa2) {
  const g = new THREE.Group();
  // big diamond, low and centred: it OWNS the middle of your rear-view mirror
  const kite = M(new THREE.PlaneGeometry(0.85, 0.85), cloth, { side: THREE.DoubleSide });
  kite.position.set(-2.0, 1.15, 0);
  kite.rotation.y = Math.PI / 2;          // faces down the car's axis
  kite.rotation.z = Math.PI / 4;          // the diamond
  g.add(kite);
  if (face) {
    // THE MIRROR GAG: painted on the INNER face only. The chase camera sits
    // behind the kite looking forward and sees plain cloth; the rear-view
    // camera looks BACK and meets the face head-on, chasing you.
    const decal = new THREE.Mesh(
      new THREE.PlaneGeometry(0.62, 0.62),
      new THREE.MeshBasicMaterial({ map: makeKiteFace(face), transparent: true, side: THREE.FrontSide, depthWrite: false })
    );
    decal.position.set(-1.985, 1.15, 0);
    decal.rotation.y = Math.PI / 2;       // normal points +X: toward the kart, toward the mirror
    g.add(decal);
  }
  // tails
  for (let k = 0; k < 3; k++) {
    const bow = M(new THREE.BoxGeometry(0.10, 0.05, 0.02), [0xffd24d, 0x2fe6c8, 0xff8c42][k]);
    bow.position.set(-2.35 - k * 0.16, 0.86 - k * 0.14, 0);
    g.add(bow);
  }
  const spool = M(new THREE.CylinderGeometry(0.05, 0.05, 0.10, 8), 0x8a5f33);
  spool.position.set(-1.05, 0.62, 0.3);
  spool.rotation.z = Math.PI / 2;
  g.add(spool);
  // it flies: gentle bob + sway
  g.userData.anim = (t) => { g.position.y = Math.sin(t * 1.7) * 0.06; g.rotation.x = Math.sin(t * 1.1) * 0.05; };
  return g;
}
function trident() {
  const g = new THREE.Group();
...
function buildAttachments(loadout = {}, skin, anims = []) {
  const grp = new THREE.Group();
  const add = (registry, id, fallback) => {
    if (!id) return;
    const make = registry[id] || fallback;
    if (!make) return;
    const m = make(idHue(id));
    grp.add(m);
    if (m.userData?.anim) anims.push(m.userData.anim);
  };
  {
    const before = grp.children.length;
    add(HEAD_SHAPES, loadout.headpiece, (c) => finShape(c));        // unknown headwear → a fin
    // hats were authored against the old helmeted head and hover over the new
    // bare one — seat every headpiece down onto the scalp
    if (grp.children.length > before) grp.children[grp.children.length - 1].position.y -= 0.06;
  }
  // MASKS sit on the NEW big head: the old anchors were tuned for the helmeted
  // 0.26 head — nudge every breather forward and up a touch so nothing sinks
  // into the cheeks.
  {
    const before = grp.children.length;
    add(BREATHER_SHAPES, loadout.breather, (c) => snorkel(c, 0xff5a3c));
    if (grp.children.length > before) {
      const m = grp.children[grp.children.length - 1];
      m.position.z += 0.045; m.position.y += 0.012; m.scale.multiplyScalar(1.06);
    }
  }
...
function buildCarAttachments(loadout = {}, anims = []) {
  const grp = new THREE.Group();
  const add = (registry, id, fallback) => {
    if (!id) return;
    const make = registry[id] || fallback;
  ...
  // GEAR IS RETIRED as a visible category — the slot now carries KITES only.
  // Legacy tools in old wardrobes simply render nothing.
  if (loadout.weapon && (loadout.weapon.startsWith("kite_") || loadout.weapon === "tool_kite"))
    add(GEAR_SHAPES, loadout.weapon, null);
...
const MYTHIC = {
  ...
  body_regalia:        { glow: 0xffb020, flames: true },        // Golden Hour — was pointing at a dead id
  shoes_comet:         { glow: 0xffc83d, trail: "comet", sparks: true },   // Comet Treads — same
};
...
export function buildCar({ bodyColor = PALETTE.toyRed, capColor = null, skin = PALETTE.skin, loadout = {} } = {}) {
  // FLAG COSMETICS. A pennant is the most visible thing on a kart from behind —
  // which is the view every rival has of you for the whole race — so it's an
  // obvious thing to sell. The colour comes from the equipped flag.
  const flagColor = FLAG_COLORS[loadout.flag] ?? null;
  const flagId = loadout.flag || null;
  const car = new THREE.Group();
  const cosmeticAnims = [];   // every animated cosmetic registers here; animateCar runs them
  ...
  // ---- BODY KITS ----
  // A shell is a KIT now, not a paint job: each one bolts real geometry onto
  // the kart so the silhouette changes at a glance.
  const applyBodyKit = (grp2) => {
    const id = loadout.body;
    const kit = new THREE.Group();
    const box = (w, h, d, mat, x, y, z, ry = 0) => {
      const m2 = new THREE.Mesh(new THREE.BoxGeometry(w, h, d), mat);
      m2.position.set(x, y, z); m2.rotation.y = ry; kit.add(m2); return m2;
    };
    if (id === "body_speedster") {                       // low race wing + side skirts
      box(0.10, 0.05, 1.30, trimMat, -1.30, 0.86, 0);
      for (const s of [1, -1]) box(0.06, 0.30, 0.05, trimMat, -1.30, 0.68, s * 0.55);
      for (const s of [1, -1]) box(1.7, 0.07, 0.08, accentMat, 0, 0.24, s * 0.66);
    } else if (id === "body_lifeguard") {                // roof light bar + rescue board
      const bar = box(0.14, 0.10, 0.9, trimMat, 0.25, 1.02, 0);
      const lamp = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.09, 0.2),
        new THREE.MeshBasicMaterial({ color: 0xff4a3c, fog: false }));
      lamp.position.set(0.25, 1.09, 0); kit.add(lamp);
      const board = box(1.5, 0.05, 0.34, accentMat, -0.35, 0.98, 0.42, 0.06);
      board.rotation.z = 0.02;
      kit.userData.anim = (t) => { lamp.material.color.setHex(Math.floor(t * 3) % 2 ? 0xff4a3c : 0xffe6e0); };
    } else if (id === "body_mecha") {                    // armor plates + antenna
      for (const s of [1, -1]) box(0.55, 0.16, 0.10, trimMat, 0.35, 0.60, s * 0.60, s * 0.15);
      box(0.4, 0.12, 0.5, trimMat, -0.9, 0.84, 0);
      const ant = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.015, 0.5, 5), accentMat);
      ant.position.set(-1.1, 1.1, -0.4); kit.add(ant);
    } else if (id === "body_ronin") {                    // storm fin + twin stacks
      const fin = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.42, 0.05), trimMat);
      fin.position.set(-1.15, 1.0, 0); fin.rotation.z = -0.35; kit.add(fin);
      for (const s of [1, -1]) {
        const st = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.08, 0.34, 8), accentMat);
        st.position.set(-0.95, 0.92, s * 0.30); st.rotation.x = s * 0.2; kit.add(st);
      }
    } else if (id === "body_tiki") {                     // bamboo trim + torches
      for (const s of [1, -1]) {
        const bam = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 1.5, 7), trimMat);
        bam.rotation.x = Math.PI / 2; bam.position.set(0.1, 0.55, s * 0.64); kit.add(bam);
        const torch = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.06, 0.4, 6), trimMat);
        torch.position.set(-1.05, 0.95, s * 0.45); kit.add(torch);
        const fl = new THREE.Mesh(new THREE.ConeGeometry(0.07, 0.16, 6),
          new THREE.MeshBasicMaterial({ color: 0xff8c42, fog: false }));
        fl.position.set(-1.05, 1.2, s * 0.45); kit.add(fl);
        (kit.userData.flames ||= []).push(fl);
      }
      kit.userData.anim = (t) => { kit.userData.flames.forEach((f, i2) => { f.scale.y = 0.8 + Math.sin(t * 13 + i2 * 2) * 0.3; }); };
    } else if (id === "body_regalia") {                  // MYTHIC: gold wing + hood bird
      const wing = box(0.12, 0.06, 1.4, accentMat, -1.32, 0.95, 0);
      for (const s of [1, -1]) box(0.08, 0.34, 0.06, accentMat, -1.32, 0.76, s * 0.62);
      const bird = new THREE.Mesh(new THREE.ConeGeometry(0.09, 0.28, 6), accentMat);
      bird.rotation.z = -Math.PI / 2.4; bird.position.set(1.35, 0.62, 0); kit.add(bird);
      kit.userData.anim = (t) => { wing.position.y = 0.95 + Math.sin(t * 2.3) * 0.02; };
    }
    if (kit.children.length) {
      grp2.add(kit);
      if (kit.userData.anim) cosmeticAnims.push(kit.userData.anim);
    }
  };
  applyBodyKit(body);
  ...
  // ---- BODY, PARTY-GAME STYLE: a plain shirt and shorts ----
  // No zips, no panels, no shoulder balls: the character reads through
  // proportion and colour. Shirt = the player's tint; shorts stay dark.
  const torso = new THREE.Mesh(new THREE.CapsuleGeometry(0.175, 0.14, 5, 10), suit);
  torso.position.y = 0.17;
  torso.castShadow = true;
  driver.add(torso);
  const shorts = new THREE.Mesh(new THREE.SphereGeometry(0.155, 10, 8), suitDark);
  shorts.position.y = 0.045;
  shorts.scale.set(1, 0.62, 0.95);
  driver.add(shorts);
  ...
  // ---- HEAD, PARTY-GAME STYLE ----
  // One big bare head with a painted face decal and a simple hair shell. The
  // head fills the anchor contract EXACTLY: centre y=0.48 and radius 0.30, so
  // its top IS the crown at 0.78 — every existing hat sits straight on the
  // scalp, no cosmetic changed. Faces come in three moods; hair in six
  // colours; both picked by the player's tint so a lobby never looks cloned.
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.30, 16, 13), skinMat);
  head.position.y = 0.48;
  head.scale.set(0.98, 1.0, 0.94);
  head.castShadow = true;
  driver.add(head);
  const hairCols = [0x5a4030, 0x2c2620, 0xd9a441, 0x3f7f8c, 0xc95f8a, 0x7a4a2e];
  const bcNum = typeof bodyColor === "number" ? bodyColor : new THREE.Color(bodyColor).getHex();
  const hairCol = capColor ?? hairCols[Math.abs((bcNum >> 4) + (bcNum >> 12)) % hairCols.length];
  const hair = new THREE.Mesh(new THREE.SphereGeometry(0.315, 16, 10, 0, Math.PI * 2, 0, Math.PI * 0.52), toon(hairCol));
  hair.position.y = 0.48;
  hair.scale.set(0.98, 1.0, 0.94);
  driver.add(hair);
  const face = new THREE.Mesh(
    new THREE.PlaneGeometry(0.34, 0.34),
    new THREE.MeshBasicMaterial({ map: makeFaceTexture((bcNum >> 8) ^ bcNum), transparent: true, depthWrite: false })
  );
  face.position.set(0, 0.465, 0.287);
  driver.add(face);
  for (const s of [-1, 1]) {
    const ear = new THREE.Mesh(new THREE.SphereGeometry(0.052, 6, 5), skinMat);
    ear.position.set(s * 0.285, 0.47, 0.0);
    ear.scale.set(0.55, 1, 0.8);
    driver.add(ear);
  }

  // seat the character in the cockpit, facing down the car's forward axis —
  // this line and the add were lost in the head surgery, which is why the
  // character vanished entirely. Never excise to a section boundary again.
  driver.rotation.y = Math.PI / 2;   // face the way the kart drives — the flip had them riding backward
  driver.position.set(-0.12, 0.52, 0);
  // THE COSMETICS, RE-WIRED. The head surgery swallowed these two calls along
  // with the seat — equipped items stopped rendering entirely and nobody's
  // wardrobe has shown on a kart since. Hats/masks/scarves ride the driver;
  // floaty/gear/tow ride the kart. Their animators land in cosmeticAnims.
  driver.add(buildAttachments(loadout, skin, cosmeticAnims));
  car.add(buildCarAttachments(loadout, cosmeticAnims));
  car.add(driver);

  // ---- THE PENNANT ----
  //
  // A PlaneGeometry's default face lies in the XY plane, so this flag hung SIDEWAYS
  // off the mast — flat to the world instead of streaming behind the kart. And the
  // animation rotated it about Y, which spun it like a weathervane rather than
  // fluttering it.
  //
  // The flag is built in the BODY's frame (forward = +Z here, before the body's
  // quarter-turn), so it must stream toward -Z. Rotate it to face across the wind.
  const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.022, 0.026, 0.95, 6), toon(0xd9d2c4));
  pole.position.set(-0.48, 1.12, -1.00);
  body.add(pole);

  // the flag itself: a group, so `animateCar` can flutter the CLOTH without
  // spinning the mast
  const flag = new THREE.Group();
  const cloth = new THREE.Mesh(
    new THREE.PlaneGeometry(0.46, 0.28, 4, 1),          // segmented, so it can ripple
    toon(flagColor ?? 0xfff1d6, { side: THREE.DoubleSide })
  );
  cloth.rotation.y = Math.PI / 2;                       // face ACROSS the airflow
  cloth.position.set(0, 0, -0.23);                      // stream backward from the mast
  flag.add(cloth);

  // ---- WHAT'S PRINTED ON IT ----
  // A flag is not a coloured rectangle. It has a DEVICE on it — that device is the
  // whole reason you'd want one. The Jolly Roger has a skull. The chequered flag
  // has squares. Without this, a "flag cosmetic" is a paint swatch.
  const emblem = FLAG_EMBLEMS[flagId];
  if (emblem) {
    const e = emblem();
    e.rotation.y = Math.PI / 2;
    e.position.set(0.012, 0, -0.23);                    // just proud of the cloth
    flag.add(e);
    const e2 = emblem();                                // and on the reverse
    e2.rotation.y = -Math.PI / 2;
    e2.position.set(-0.012, 0, -0.23);
    flag.add(e2);
  }

  // a bright hem, so it reads at speed
  const hem = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.28, 0.02), toon(0xe2574c));
  hem.position.set(0, 0, -0.46);
  flag.add(hem);
  flag.position.set(-0.48, 1.42, -1.00);
  body.add(flag);
  ...
  for (const [wx, wz] of [[0.62, 0.72], [-0.62, 0.72], [0.62, -0.78], [-0.62, -0.78]]) {
    const w = buildWheel(loadout.shoes);
    if (w.userData.anim) cosmeticAnims.push(w.userData.anim);
  ...
  // ---- NEON UNDERGLOW + AFTERBURNER ----
  //
  // The two things a stylised night racer's kart cannot be without. The
  // underglow is a soft additive pool in the LIVERY colour — it plants the kart
  // on the dark road, doubles as team identity at a glance, and surges on a
  // turbo. The afterburner is a cyan-white cone off the tail that only exists
  // while a boost burns (animateCar drives both).
  const glowTex = getUnderglowTexture();
  const underglow = new THREE.Mesh(
    new THREE.PlaneGeometry(4.6, 3.0),
    new THREE.MeshBasicMaterial({
      map: glowTex, color: bodyColor, transparent: true, opacity: 0.42,
  ...
  const boostFlame = new THREE.Mesh(
    new THREE.ConeGeometry(0.24, 1.5, 8, 1, true),
    new THREE.MeshBasicMaterial({
      color: 0x7deeff, transparent: true, opacity: 0,
      blending: THREE.AdditiveBlending, depthWrite: false, fog: false,
    })
  );
  // AXIS-LOCKED TO THE RING: the flame is a child of the engine-ring group and
  // fires along the ring's own axis — it cannot end up on the side of the car
  // while the ring is on the back, because they share one orientation.
  boostFlame.rotation.z = Math.PI / 2;
  boostFlame.position.set(-0.78, 0, 0);

  // ---- THE ENGINE RING ----
  // The rear of every kart carries a big circular engine core: dark housing, a
  // glowing ring, and an inner disc that idles cyan and IGNITES when a turbo
  // burns (the boost flame fires out of its centre). It's the one piece of
  // kart furniture that reads from any distance, on every livery — the thing
  // your eye locks onto when you're chasing someone down.
  // the big cyan ring is REMOVED by request — the group survives, empty, as
  // the mount that keeps the boost flame aimed straight out the back.
  const ringGrp = new THREE.Group();
  ringGrp.add(boostFlame);
  ringGrp.position.set(-1.42, 0.55, 0);
  car.add(ringGrp);

  car.userData = { wheels, driver, flag, baseY: 0, auras, mythic: myth.length > 0, erosion, shell, erosionLevel: 0, underglow, boostFlame, cosmeticAnims };
  return car;
}


// Cached radial pool for every kart's underglow — one canvas, drawn once.
let _underglowTex = null;
...
export function animateCar(car, { speed = 0, steer = 0, offTrack = false, erosion = 0, turbo = false }, dt) {
  const { wheels, driver, flag, auras, erosion: ero, shell, underglow, boostFlame } = car.userData;

  // ---- UNDERGLOW + AFTERBURNER ----
  // The glow pool brightens with pace and SURGES on a turbo; the flame only
  // exists while the boost burns, flickering in length so it reads as fire and
  // not a decal. Both ease rather than switch, so a boost ends with a fade.
  if (underglow) {
    const wantU = 0.34 + Math.min(0.28, (Math.abs(speed) / 26) * 0.28) + (turbo ? 0.30 : 0);
    underglow.material.opacity += (wantU - underglow.material.opacity) * Math.min(1, dt * 6);
  }
  if (boostFlame) {
    const wantF = turbo ? 0.85 : 0;
    boostFlame.material.opacity += (wantF - boostFlame.material.opacity) * Math.min(1, dt * (turbo ? 10 : 5));
    if (boostFlame.material.opacity > 0.03) {
      const t = performance.now() / 1000;
      const len = 1 + Math.sin(t * 27) * 0.18 + (turbo ? 0.25 : 0);
      boostFlame.scale.set(1, len, 1);
  ...
  // ---- COSMETIC ANIMATORS ----
  // Premium pieces move: halos spin, comets pulse, propellers whirl. Each
  // registered a closure at build time; run them all with one clock.
  if (car.userData.cosmeticAnims && car.userData.cosmeticAnims.length) {
    const tA = performance.now() / 1000;
    for (const fn of car.userData.cosmeticAnims) { try { fn(tA, dt); } catch {} }
  }
  ...
  // THE FLAG FLUTTERS. It used to ROTATE ABOUT Y — spinning like a weathervane,
  // which is not a thing a flag does. A pennant in the airflow snaps taut as you
  // speed up and RIPPLES; it doesn't pirouette.
  if (flag) {
    const t = performance.now() * 0.006;
    const s01 = Math.min(1, Math.abs(speed) / 26);
    // it swings side to side, hard at low speed, tight and snappy at high speed
    flag.rotation.y = Math.sin(t * 2.2) * (0.42 - s01 * 0.30);
    // and it lifts as the airflow catches it
    flag.rotation.z = -s01 * 0.5 + Math.sin(t * 3.1) * 0.10;
    // the cloth itself ripples
    const cloth = flag.children[0];
    if (cloth) cloth.rotation.x = Math.sin(t * 4.4) * (0.06 + s01 * 0.10);
  }
  for (const w of wheels) {
    ...
    // NEGATED, because the whole car mesh lives in a flipped yaw frame
    // (`car.rotation.y = -heading`) — the body roll and driver lean below already
    // use `-steer` for exactly this reason. Without the minus, the front wheels
    // pointed OPPOSITE the way you were turning: steer right, wheels crank left.
...
// ============================================================================
// THE COSMETIC SELF-TEST.
// Instantiates EVERY registered builder — all hats, masks, scarves, floats,
// gear, tow pieces, every wheel — plus one full premium buildCar, inside
// try/catch, and reports exactly which ids fail. This is what "every item
// works" means as a gate instead of a hope. Run from the browser console or
// the QA probe via window.__cosTest().
// ============================================================================
export function __cosSelfTest() {
  const fails = [];
  let total = 0;
  const tryOne = (id, fn) => {
    total++;
    try { const g = fn(); if (!g || !g.isObject3D) throw new Error("no object"); }
    catch (e) { fails.push(id + ": " + (e?.message || e)); }
  };
  for (const [id, fn] of Object.entries(HEAD_SHAPES)) tryOne(id, () => fn(0xff5a4c));
  for (const [id, fn] of Object.entries(BREATHER_SHAPES)) tryOne(id, () => fn(0x2fb9c9));
  for (const [id, fn] of Object.entries(SCARF_SHAPES)) tryOne(id, () => fn(0xffd24d));
  for (const [id, fn] of Object.entries(FLOATY_SHAPES)) tryOne(id, () => fn(0x2fe6c8));
  for (const [id, fn] of Object.entries(GEAR_SHAPES)) tryOne(id, () => fn(0x8a5f33));
  for (const [id, fn] of Object.entries(BELT_SHAPES)) tryOne(id, () => fn(0x8a5f33));
  for (const id of ["shoes_standard","shoes_sandal","shoes_neon","shoes_turbo","shoes_chrome","shoes_coral","shoes_storm","shoes_comet","shoes_wood"]) tryOne(id, () => buildWheel(id));
  for (const id of ["tool_kite","kite_clown","kite_shark","kite_eyes"]) tryOne(id, () => GEAR_SHAPES[id](0xff5fa2));
  for (const id of ["body_standard","body_speedster","body_lifeguard","body_mecha","body_ronin","body_tiki","body_regalia"]) tryOne("kit:" + id, () => buildCar({ bodyColor: 0xe2574c, loadout: { body: id } }));
  for (const id of ["head_crown","shoes_comet","kite_shark","tank_jetpack","flag_checker"]) tryOne("thumb:" + id, () => buildCosmeticPreview(id));
  tryOne("buildCar(full-premium)", () => buildCar({ bodyColor: 0xff5a4c, loadout: {
    headpiece: "head_marshal", breather: "breather_kraken", bandana: "bandana_trailblazer",
    oxygenTank: "tank_jetpack", weapon: "tool_naginata", belt: "belt_cooler",
    body: "body_regalia", shoes: "shoes_comet", flag: "flag_checker",
  }}));
  return { total, fails };
}

// ============================================================================
// ONE PREVIEW OBJECT FOR ANY COSMETIC ID — feeds the generated thumbnails.
// ============================================================================
export function buildCosmeticPreview(id) {
  if (!id) return null;
  const hue = idHue(id);
  if (HEAD_SHAPES[id]) { const g = HEAD_SHAPES[id](hue); g.position.y -= 0.45; return g; }
  if (BREATHER_SHAPES[id]) { const g = BREATHER_SHAPES[id](hue); g.position.y -= 0.35; return g; }
  if (SCARF_SHAPES[id]) { const g = SCARF_SHAPES[id](hue); g.position.y -= 0.2; return g; }
  if (FLOATY_SHAPES[id]) return FLOATY_SHAPES[id](hue);
  if (GEAR_SHAPES[id]) return GEAR_SHAPES[id](hue);
  if (BELT_SHAPES[id]) return BELT_SHAPES[id](hue);
  if (id.startsWith("shoes_")) return buildWheel(id);
  if (id.startsWith("body_")) return buildCar({ bodyColor: 0xe2574c, loadout: { body: id } });
  if (id.startsWith("flag_")) {
    const g = new THREE.Group();
    const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 1.2, 6), toon(0xd8dde3));
    const cloth = new THREE.Mesh(new THREE.PlaneGeometry(0.7, 0.45), toon(FLAG_COLORS[id] ?? idHue(id), { side: THREE.DoubleSide }));
    cloth.position.set(0.36, 0.35, 0);
    pole.position.y = 0;
    g.add(pole, cloth);
    return g;
  }
