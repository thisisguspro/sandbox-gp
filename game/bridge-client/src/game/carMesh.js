...
export function buildCar({ bodyColor = PALETTE.toyRed, capColor = null, skin = PALETTE.skin, loadout = {} } = {}) {
  ...
  boostFlame.rotation.z = Math.PI / 2;   // tip points -X: straight out the back
  boostFlame.position.set(-1.62, 0.55, 0);
  car.add(boostFlame);
  ...
  // the number plate under the ring — SANDBOX GP, like every kart shipped from
  // the same works team
  const plate = new THREE.Mesh(
    new THREE.PlaneGeometry(0.62, 0.16),
    new THREE.MeshBasicMaterial({ map: getPlateTexture(), fog: false })
  );
  plate.rotation.y = -Math.PI / 2;
  plate.position.set(-1.46, 0.26, 0);
...
// One cached rear-plate texture for every kart.
let _plateTex = null;
function getPlateTexture() {
  ...
  g.fillStyle = "#120c1c"; g.fillRect(0, 0, 128, 32);
  g.strokeStyle = "#fff2e8"; g.lineWidth = 2; g.strokeRect(2, 2, 124, 28);
  g.fillStyle = "#fff2e8";
  g.font = "900 15px sans-serif";
  g.textAlign = "center"; g.textBaseline = "middle";
  g.fillText("SANDBOX GP", 64, 17);
