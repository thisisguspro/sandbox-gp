...
export function buildCar({ bodyColor = PALETTE.toyRed, capColor = null, skin = PALETTE.skin, loadout = {} } = {}) {
  ...
  const hairCol = capColor ?? hairCols[Math.abs((bodyColor >> 4) + (bodyColor >> 12)) % hairCols.length];
  ...
  const face = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ map: makeFaceTexture((bodyColor >> 8) ^ bodyColor), transparent: true, depthWrite: false })
  );
  ...
  housing.rotation.z = Math.PI / 2;      // axis along X, facing rearward
  ringGrp.add(housing);
  const glowRing = new THREE.Mesh(
    new THREE.TorusGeometry(0.30, 0.045, 8, 24),
    new THREE.MeshBasicMaterial({ color: 0x22e6ff, transparent: true, opacity: 0.9, blending: THREE.AdditiveBlending, depthWrite: false, fog: false })
  );
  glowRing.rotation.y = Math.PI / 2;     // ring plane faces rearward
  glowRing.position.x = -0.08;
  ringGrp.add(glowRing);
  const ringCore = new THREE.Mesh(
    new THREE.CircleGeometry(0.24, 18),
    new THREE.MeshBasicMaterial({ color: 0x66eaff, transparent: true, opacity: 0.22, blending: THREE.AdditiveBlending, depthWrite: false, fog: false, side: THREE.DoubleSide })
  );
  ringCore.rotation.y = -Math.PI / 2;
  ringCore.position.x = -0.09;
  ringGrp.add(ringCore);
  ringGrp.add(boostFlame);
  ringGrp.position.set(-1.42, 0.55, 0);
  car.add(ringGrp);

  car.userData = { wheels, driver, flag, baseY: 0, auras, mythic: myth.length > 0, erosion, shell, erosionLevel: 0, underglow, boostFlame, ringCore, glowRing };
  return car;
}
...
export function animateCar(car, { speed = 0, steer = 0, offTrack = false, erosion = 0, turbo = false }, dt) {
  ...
  // the engine core: idles a faint cyan, IGNITES white-hot with the turbo
  {
    const { ringCore, glowRing } = car.userData;
    if (ringCore) {
      const wantC = turbo ? 0.95 : 0.20 + Math.min(0.18, (Math.abs(speed) / 26) * 0.18);
      ringCore.material.opacity += (wantC - ringCore.material.opacity) * Math.min(1, dt * 8);
    }
    if (glowRing) glowRing.material.opacity = turbo ? 1.0 : 0.9;
  }
