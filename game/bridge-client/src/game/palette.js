...
// The character's FACE — a flat painted decal in the party-game tradition:
// big oval eyes with a highlight, bold brows, a small mouth. Three moods,
// picked by seed, so a grid of racers doesn't share one soul.
export function makeFaceTexture(seed = 1) {
  const px = 128;
  const c = document.createElement("canvas"); c.width = c.height = px;
  const g = c.getContext("2d");
  const mood = Math.abs(seed | 0) % 3;   // 0 determined · 1 happy · 2 smug
  g.clearRect(0, 0, px, px);
  const eye = (cx) => {
    g.fillStyle = "#22201d";
    g.beginPath(); g.ellipse(cx, 62, 11, mood === 1 ? 15 : 13, 0, 0, Math.PI * 2); g.fill();
    g.fillStyle = "#ffffff";
    g.beginPath(); g.ellipse(cx + 3.5, 56, 3.6, 4.6, 0, 0, Math.PI * 2); g.fill();
  };
  eye(44); eye(84);
  g.strokeStyle = "#22201d"; g.lineWidth = 6; g.lineCap = "round";
  const browTilt = mood === 0 ? 8 : mood === 2 ? -6 : 0;
  g.beginPath(); g.moveTo(30, 40 + browTilt); g.lineTo(56, 36); g.stroke();
  g.beginPath(); g.moveTo(72, 36); g.lineTo(98, 40 + browTilt); g.stroke();
  g.lineWidth = 5;
  g.beginPath();
  if (mood === 1) { g.arc(64, 88, 10, 0.15 * Math.PI, 0.85 * Math.PI); }
  else if (mood === 2) { g.moveTo(56, 92); g.quadraticCurveTo(70, 96, 78, 88); }
  else { g.moveTo(55, 92); g.lineTo(74, 92); }
  g.stroke();
...
export function makeGroundMaps(base = PALETTE.sandLight, px = 256, seed = 777, bump = 1.6) {
  ...
  // HAND-PAINTED, NOT NOISED. The old albedo was per-pixel value noise —
  // sandpaper. This is a painter's pass: a warm base coat, then a few hundred
  // soft tinted blobs (hue drifted a little each way, like brush loads), then
  // sparse darker dabs. Tiles seamlessly because blobs wrap at the edges.
  ...
  col.getHSL(hsl);
  ag.fillStyle = col.getStyle();
  ag.fillRect(0, 0, px, px);
  const blob = (cx, cy, r, fill) => {
    const g2 = ag.createRadialGradient(cx, cy, 0, cx, cy, r);
    g2.addColorStop(0, fill);
    g2.addColorStop(1, "rgba(0,0,0,0)");
    ag.fillStyle = g2;
    for (const ox of [-px, 0, px]) for (const oy of [-px, 0, px]) {
      ag.beginPath(); ag.arc(cx + ox, cy + oy, r, 0, Math.PI * 2); ag.fill();
    }
  };
  const c2 = new THREE.Color();
  for (let i = 0; i < 170; i++) {
    c2.setHSL((hsl.h + (rnd2() - 0.5) * 0.045 + 1) % 1, Math.min(1, hsl.s * (0.85 + rnd2() * 0.4)), Math.min(1, hsl.l * (0.86 + rnd2() * 0.30)));
    blob(rnd2() * px, rnd2() * px, (0.03 + rnd2() * 0.09) * px, c2.getStyle().replace("rgb", "rgba").replace(")", ",0.30)"));
  }
  for (let i = 0; i < 46; i++) {
    c2.setHSL((hsl.h + (rnd2() - 0.5) * 0.03 + 1) % 1, hsl.s, Math.max(0, hsl.l * 0.68));
    blob(rnd2() * px, rnd2() * px, (0.010 + rnd2() * 0.028) * px, c2.getStyle().replace("rgb", "rgba").replace(")", ",0.28)"));
  }
