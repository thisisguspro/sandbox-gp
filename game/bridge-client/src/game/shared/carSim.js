...
export const CAR = {
  ...
  TURN_DRAG_FLAT: 1.1,    // and a FLAT cost for holding A/D at ANY speed — looping the wheel is never free
...
export function stepCarFree(state, input, dt, mods = {}) {
  ...
  if (state.speed > 0) {
    state.speed = Math.max(0, state.speed - (CAR.TURN_DRAG * spdFrac + CAR.TURN_DRAG_FLAT) * Math.abs(str) * dt);
  }
...
export function stepCar(state, input, dt, track, mods = {}) {
  ...
  let ground = 0;
  let roadY = smp.y || 0;   // computed below for ANY non-gap sample, ribbon or not
  if (!smp.gap && track.samples) {
    ...
    if (nxt && !nxt.gap) {
      ...
      if (far && !far.gap) {
        ...
        roadY = (smp.y || 0) + grade * along;
      }
    }
  }
  // THE FLUSH BORDER. On the paint OR the 3m shoulder beside it, the ground is
  // the road's own height — riding the red-and-white can never drop you under
  // the deck (that was the fall-through: one pixel past the curb and ground
  // snapped to the beach, and the glue teleported you down THROUGH the track).
  // Past the shoulder the ground really is the beach below.
  const SHOULDER = 3.0;
  if (!smp.gap) {
    if (Math.abs(lat) <= half + SHOULDER) ground = roadY;
    else ground = 0;
  } else ground = 0;
  ...
  // this function.
    ...
    if (ground < state.y - 0.45) {
      // the floor fell away (bridge edge, shoulder end): become airborne and
      // FALL — no invisible deck-level floor, no teleport through geometry
      state.airborne = true;
      state.vy = 0;
    } else {
  ...
  if (state.airborne) {
    ...
    if (state.y <= ground) {
      ...
      state.hardLanding = fall;                          // the engine turns big falls into fall DAMAGE
    }
  }
  ...
  state.hazContact = false;   // the engine turns any contact into ONE discrete hit
  for (const h of track.def?.hazards || []) {
    const d = Math.hypot(state.x - h.x, state.z - h.z);
    if (d > h.r) continue;
    state.hazContact = true;
    state.inHazard = h.kind;
    if (h.kind === "oil") {
      // an oil slick: you keep your speed and lose your steering. Terrifying.
      state.hazardGrip = 0.25;
      // OIL STEALS THE WHEEL: a deterministic wobble (same on server and in
      // prediction) yanks the heading while you're on the slick — you overturn
      // when you didn't ask to. Scales with speed; fades the moment you're off.
      state.oilT = (state.oilT || 0) + dt;
      state.heading += Math.sin(state.oilT * 12.5) * 1.35 * Math.min(1, Math.abs(state.speed) / 18) * dt;
