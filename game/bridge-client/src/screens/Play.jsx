    ...
    <div style={{ height: "100%", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 2, height: "100%", display: "grid", gridTemplateColumns: "1fr 360px" }}>
        ...
        <div style={{ padding: "20px 40px 24px", display: "flex", flexDirection: "column", minHeight: 0, overflowY: "auto" }}>
          ...
          <div className="faint" style={{ fontSize: 11, marginBottom: 8 }}>{t("play.hud.karmaHint")} Click a row for the lap-by-lap breakdown.</div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr className="faint" style={{ textAlign: "left", fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase" }}>
                  ...
                  <th style={{ padding: "6px 8px" }}>Best lap</th>
                  <th style={{ padding: "6px 8px" }}>Laps</th>
                  <th style={{ padding: "6px 8px" }}></th>
                </tr>
              </thead>
              <tbody>
                    ...
                    <FragmentRow key={p.id} open={open}
                      row={
                        <tr onClick={() => setLapView(open ? null : p.id)}
                          ...
                          <td className="impactf" style={{ padding: "8px 8px" }}>{total != null && p.finished !== false ? total.toFixed(2) + "s" : total != null ? total.toFixed(2) + "s" : "—"}</td>
                          <td className="impactf" style={{ padding: "8px 8px", color: "var(--volt)" }}>{best != null ? best.toFixed(2) + "s" : "—"}</td>
                          <td style={{ padding: "8px 8px" }}>{laps.length}</td>
                      ...
                      detail={
                        <tr style={{ background: "var(--ink-3)" }}>
                          <td colSpan={6} style={{ padding: "6px 14px 12px" }}>
                            {laps.length === 0 ? (
