...
export default function Hangar({ user, profile, catalogue }) {
  ...
  return (
    <div style={wrap}>
      <SpeedLines />
      {/* the garage hero: YOUR kart, with everything you've equipped — drag to turn it */}
      <div style={{ maxWidth: 560, margin: "0 auto 6px" }}>
        <KartPreview loadout={profile.loadout || {}} idColor={profile.idColor || "#e2574c"} height={230} />
      </div>
                    ...
                    return (
                      <div key={id} className="row gap-s" style={{ fontSize: 12, fontWeight: 600, opacity: reached ? 1 : 0.55, alignItems: "center" }}>
                        <span style={{ width: 28, height: 28, flexShrink: 0, display: "grid", placeItems: "center", background: `radial-gradient(circle at 50% 40%, ${rarityColor(c?.rarity)}22 0%, transparent 70%)`, border: "1px solid var(--line)" }}>
                          <CosThumb id={c?.id} size={30} style={{ opacity: reached ? 1 : 0.4 }} fallback={<ItemIcon id={c?.id} slot={c?.slot} color={rarityColor(c?.rarity)} glyphSize={14} size="78%" dim={!reached} />} />
                        </span>
                        <span>{c?.name || id}</span>
                      </div>
                    );
...
const cardNext = { opacity: 1, borderColor: "var(--hot-deep)", boxShadow: "0 0 0 1px rgba(255,45,77,0.25), 0 8px 30px rgba(120, 90, 50, 0.4)" };
