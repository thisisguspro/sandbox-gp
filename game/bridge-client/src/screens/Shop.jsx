...
import CosThumb from "../components/CosThumb.jsx";
...
export default function Shop({ profile, catalogue, onChange }) {
  ...
  const [justGot, setJustGot] = useState(null);   // { id, name } → equip-now banner
  ...
  const buy = async (it, e) => {
    ...
    try {
      ...
      setJustGot({ id: it.id, name: it.name });   // → the "equip now?" banner
      await onChange?.();
    ...
    <div style={wrap}>
      ...
      <div style={{ position: "relative", zIndex: 2, height: "100%", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", gap: 2, padding: "16px 36px 0", borderBottom: "2px solid var(--line)" }}>
          {justGot && (
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(47,230,200,0.14)", border: "1px solid rgba(47,230,200,0.5)", borderRadius: 12, padding: "8px 12px", marginBottom: 10 }}>
              <span style={{ fontWeight: 800 }}>Got it: {justGot.name}. Equip now?</span>
              <button className="btn" style={{ padding: "5px 14px" }} onClick={async () => { try { await api.equip(justGot.id); } catch {} setJustGot(null); }}>EQUIP</button>
              <button className="btn ghost" style={{ padding: "5px 10px" }} onClick={() => setJustGot(null)}>LATER</button>
            </div>
          )}
...
function ItemCard({ it, cosmetic, owned, busy, cur, canAfford, onBuy, onHover }) {
  ...
  return (
    <div className="neon-card" style={{ padding: 14, position: "relative", textAlign: "center", "--card-glow": rc }}
      onMouseEnter={() => canTry && onHover({ slot: cosmetic.slot, id: cosmetic.id, name: it.name })}>
      {/* rarity crown line + the item on a glow plinth: the collectible-card
          read — dark glass, the rarity's neon doing all the talking */}
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: rc, boxShadow: `0 0 10px ${rc}` }} />
      <div style={{ height: 90, display: "grid", placeItems: "center", background: `radial-gradient(circle at 50% 40%, ${rc}33 0%, transparent 70%)`, border: `1px solid ${rc}55`, marginTop: 8 }}>
        <CosThumb id={cosmetic?.id} size={44} fallback={<ItemIcon id={cosmetic?.id} slot={cosmetic?.slot} color={rc} glyphSize={34} />} />
      </div>
      <div className="impactf" style={{ fontSize: 13, marginTop: 10, color: "#fff2e8" }}>{it.name}</div>
      <div style={{ fontSize: 10, fontWeight: 700, color: rc, letterSpacing: "0.08em", textShadow: `0 0 8px ${rc}88` }}>{(it.rarity || "Common").toUpperCase()}</div>
...
function BoxCard({ box, busy, balance, onOpen, onPreview }) {
  ...
  const [showAll, setShowAll] = useState(false);   // the full drop table, collapsed
  return (
    <div className="wood-panel panel-hot" style={{ padding: 16, position: "relative" }}>
      ...
      {/* ---- WHAT'S INSIDE ----
          ...
          This rendered EVERY drop in a flat, unbounded list. The Treasure Chest has
          EIGHTY-ONE items in it — so the card grew an eighty-one-row list that shoved
          the OPEN button clean off the bottom. You could not buy the thing.
      
          A drop table is reference material, not the headline. The card shows the
          ODDS BY RARITY (which is what you actually decide on) and a handful of the
          best items; the full list is one click away and scrolls in its own box. The
          BUY BUTTON is always visible, because that is the entire point of the card. */}
      <div style={{ marginTop: 8, marginBottom: 12 }}>
        {/* the rarity odds — five rows, always */}
        {(() => {
          // The store sends `drops`, each with a WEIGHT. The card was reading
          // `box.odds` with a `chance` — fields that do not exist — so it rendered
          // nothing. Turn the weights into real percentages, which is the number a
          // player actually decides on.
          const drops = box.drops || box.odds || [];
          const total = drops.reduce((sum, d) => sum + (d.weight ?? d.chance ?? 1), 0) || 1;
          const byRarity = {};
          for (const o of drops) {
            const pct = ((o.weight ?? o.chance ?? 1) / total) * 100;
            byRarity[o.rarity] = (byRarity[o.rarity] || 0) + pct;
          }
          const ORDER = ["Common", "Rare", "Epic", "Legendary", "Mythic"];
          return (
            <div className="col" style={{ gap: 3 }}>
              {ORDER.filter((r) => byRarity[r]).map((r) => (
                <div key={r} className="row" style={{ justifyContent: "space-between", alignItems: "center", fontSize: 11 }}>
                  <span className="row gap-s" style={{ alignItems: "center" }}>
                    <span style={{ width: 7, height: 7, borderRadius: 2, background: rarityColor(r), display: "inline-block" }} />
                    <span style={{ color: "var(--paper)", fontWeight: 700 }}>{r}</span>
                  </span>
                  <span className="faint">{byRarity[r].toFixed(1)}%</span>
                </div>
              ))}
            </div>
          );
        ...
        {/* and a link to the FULL list, which scrolls in its own panel */}
        <button
        ...
        >
          {showAll ? "Hide" : `See all ${(box.drops || box.odds || []).length} items`}
        </button>

        {showAll && (
          <div className="col gap-s" style={{
            marginTop: 6,
            maxHeight: 180,                 // it SCROLLS. It does not grow the card.
            overflowY: "auto",
            padding: "6px 4px",
            borderRadius: 6,
            ...
            {(box.drops || box.odds || []).map((o, i) => (
              <button key={i}
                ...
                onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(255,255,255,0.5)"; e.currentTarget.style.borderColor = "var(--line)"; }}
                ...
                <span className="faint">
                  {(() => {
                    const drops = box.drops || box.odds || [];
                    const total = drops.reduce((s, d) => s + (d.weight ?? d.chance ?? 1), 0) || 1;
                    return (((o.weight ?? o.chance ?? 1) / total) * 100).toFixed(1);
                  })()}%
                </span>
              </button>
            ))}
    ...
    <div style={revealWrap} onClick={onClose}>
      ...
      <div style={{ position: "relative", textAlign: "center", animation: "revealpop 0.5s cubic-bezier(.2,.9,.2,1)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ width: 220, height: 220, margin: "16px auto", display: "grid", placeItems: "center",
          ...
          <CosThumb id={cosmetic?.id} size={140} fallback={<ItemIcon id={cosmetic?.id} slot={cosmetic?.slot} color={rc} size="80%" glyphSize={90} />} />
        </div>
          ...
          return (
            <div key={m.id} className="panel" style={{ padding: 14, position: "relative", opacity: locked ? 0.7 : 1 }}>
              ...
              <div style={{ height: 84, display: "grid", placeItems: "center", background: `radial-gradient(circle at 50% 40%, ${rc}22 0%, transparent 70%)`, border: "1px solid var(--line)" }}>
                <CosThumb id={cosmetic?.id} size={42} style={{ opacity: locked ? 0.45 : 1 }} fallback={<ItemIcon id={cosmetic?.id} slot={cosmetic?.slot} color={rc} glyphSize={32} dim={locked} />} />
              </div>
