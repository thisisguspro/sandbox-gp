...
export default function Locker({ profile, catalogue, onChange }) {
  ...
  const [justGot, setJustGot] = useState(null);   // { id, name } → the "equip now?" banner   // { text, bad }
  ...
  const [setFilter, setSetFilter] = useState(null);   // click a collection to filter to it
  ...
  const doCraft = async (item) => {
    try {
      ...
      // crafted → offer to wear it right away
      if (r && !r.error) setJustGot({ id: item.id, name: item.name });
  ...
  // GEAR IS RETIRED: the weapon slot carries KITES only now — legacy tools are
  // hidden from the wardrobe (still owned, just not offered).
  const kiteOnly = (it) => it.slot !== "weapon" || it.id.startsWith("kite_") || it.id === "tool_kite";
  const slotUnlocked = profile.level >= (activeSlotDef?.unlockLevel || 1);
  // filter to a collection when one is selected
  const allInSlot = bySlot[slot] || [];
  const items = (setFilter ? allInSlot.filter((c) => c.set === setFilter) : allInSlot).filter(kiteOnly);
  ...
  // NO SLOT COVERING. Equipping a kart shell used to LOCK your headgear and scarf
  // until you took it off — which makes it impossible to build a look, and building
  // a look is the entire point. Wear a Mecha Frame with a Paper Crown if you want:
  // that combination IS the expression.
  const slotCovered = false;
        ...
        <div style={{ display: "flex", flexDirection: "column", minHeight: 0 }}>
          ...
          <div style={{ padding: "18px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              ...
              {justGot && (
                <div style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(47,230,200,0.14)", border: "1px solid rgba(47,230,200,0.5)", borderRadius: 12, padding: "8px 12px", marginBottom: 8 }}>
          ...
          {/* ---- THE SETS ----
              A wardrobe of 110 loose items gives a player no reason to open
              another chest. A wardrobe where you can SEE that you're 9 of 11 into
              the Lifeguard set gives them a very good one. This is the whole
              monetisation loop, and it's the loop the research says works:
              "the clever packaging of SETS of items appeals to players." */}
          {(catalogue.sets || []).length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div className="impactf" style={{ fontSize: 11, letterSpacing: "0.14em", color: "var(--dim)", marginBottom: 8 }}>
                COLLECTIONS
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {(catalogue.sets || []).map((s) => {
                  const inSet = (catalogue.cosmetics || []).filter((c) => c.set === s.id);
                  const have = inSet.filter((c) => owned.has(c.id)).length;
                  const done = have === inSet.length && inSet.length > 0;
                  const active = setFilter === s.id;
                  return (
                    <button
                      key={s.id}
                      onClick={() => setSetFilter(active ? null : s.id)}
                      title={s.blurb}
                      style={{
                        padding: "8px 12px",
                        borderRadius: 10,
                        ...
                        minWidth: 132,
                        background: active
                          ? `linear-gradient(180deg, ${s.color}28, ${s.color}12)`
                          : "linear-gradient(180deg, #ffffff, #fff3dc)",
                        border: `2px solid ${active || done ? s.color : "var(--line)"}`,
                        boxShadow: done
                          ? `0 3px 0 ${s.color}, 0 6px 14px rgba(120,90,50,0.18)`
                          : "0 3px 0 var(--line), 0 6px 12px rgba(120,90,50,0.12)",
                      }}
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                        <span style={{ fontWeight: 800, fontSize: 12.5, color: "var(--paper)" }}>{s.name}</span>
                        {done && <span style={{ fontSize: 11 }}>✓</span>}
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 5 }}>
                        <div style={{ flex: 1, height: 5, borderRadius: 3, background: "rgba(224,192,138,0.55)", overflow: "hidden" }}>
                          <div style={{ width: `${inSet.length ? (have / inSet.length) * 100 : 0}%`, height: "100%", background: s.color }} />
                        </div>
                        <span className="impactf" style={{ fontSize: 10, color: done ? s.color : "var(--dim)" }}>
                          {have}/{inSet.length}
                        </span>
                      </div>
                    </button>
          ...
          <div style={grid} onMouseLeave={() => setTryOn(null)}>
            {/* "(None)" tile — unequip this slot, for slots that support being
                empty; alwaysFilled slots like Gear, Shades and Float always have something). */}
            ...
            {items.map((item) => {
              ...
              return (
                <button key={item.id} onClick={(e) => { if (canEquip) onEquip(item, e); if (item.slot === "emote") setEmoteNonce((n) => n + 1); }} disabled={busy === item.id}
                  ...
                  {/* which collection this belongs to — an item with no context is
                      just a hat; an item that says "Lifeguard" is a piece of
                      something you're building */}
                  {item.set && (() => {
                    const sd = (catalogue.sets || []).find((s) => s.id === item.set);
                    if (!sd) return null;
                    return (
                      <div style={{
                        ...
                        boxShadow: "0 1px 3px rgba(120,90,50,0.3)",
                      }}>{sd.name}</div>
                    );
                  })()}
                  <div style={{ ...rarityBar, background: rarityColor(item.rarity) }} />
                  <div style={cardArt(item.rarity)}>
                    <CosThumb id={item.id} size={46} style={{ opacity: isOwned ? 1 : 0.4 }} fallback={<ItemIcon id={item.id} slot={item.slot} color={rarityColor(item.rarity)} dim={!isOwned} />} />
                  </div>
      ...
      {confirmScrap && (
        <div onClick={() => setConfirmScrap(null)}
          ...
          <div onClick={(e) => e.stopPropagation()} className="leather-panel"
            ...
            <div className="display" style={{ fontSize: 24, color: "var(--paper)" }}>SCRAP {confirmScrap.name.toUpperCase()}?</div>
