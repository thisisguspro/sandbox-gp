...
export default function News({ onUnreadChange }) {
  ...
  return (
    <div style={{ padding: "32px 40px", height: "100%", overflowY: "auto" }}>
      ...
      <div className="display" style={{ fontSize: 40, color: "#fff", lineHeight: 0.95, marginTop: 2, textTransform: "uppercase" }}>{t("news.title")}</div>
          ...
          {items.map((n) => (
            <button key={n.slot} onClick={() => openTile(n.slot)} disabled={busy} style={tile}>
              ...
              <div style={{ padding: "14px 16px", textAlign: "left" }}>
                <div className="display" style={{ fontSize: 20, color: "#fff", lineHeight: 1.05 }}>{n.title || t("news.untitled")}</div>
      ...
      <div style={modal} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", borderBottom: "2px solid var(--line)" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="display" style={{ fontSize: 24, color: "#fff", lineHeight: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.title || t("news.untitled")}</div>
...
const backdrop = { position: "fixed", inset: 0, zIndex: 9990, background: "rgba(0,0,0,0.72)", display: "grid", placeItems: "center", padding: 24 };
const modal = { width: "min(860px, 96vw)", maxHeight: "92vh", overflowY: "auto", background: "var(--ink-2)", border: "2px solid var(--hot)", boxShadow: "0 20px 70px rgba(0,0,0,0.7)" };
