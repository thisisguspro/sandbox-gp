  ...
  const sameKeys = (a, b) => a && b && KEY_FIELDS.every((k) => !!a[k] === !!b[k]);
    ...
    function frame(now) {
      ...
      for (const [id, rec] of meshes) {
        ...
        animateCar(rec.mesh, { speed, steer: steerFor || 0, offTrack, erosion: eroNow }, dt);
      }
      ...
      // they need a sound that runs while they're happening.
      {
        ...
        // the trickle of water while you're painting
        if (my?.isDrawer && keys.Space && tNowMs > (art.pourAt ?? 0)) {
          sfx.waterPour?.();
      ...
      // roll bar.
      camYRef.current += ((me.y || 0) - camYRef.current) * Math.min(1, dt * 6);
      const up = 2.1 + spd01 * 0.35 + camYRef.current;
      ...
      if (!camSeededRef.current && me.seeded) {
        camYRef.current = me.y || 0;      // seed the smoothed height, or it lerps up from zero
        cam.position.set(cx, 2.1 + spd01 * 0.35 + camYRef.current, cz);
      ...
      cam.lookAt(
        ...
        0.95 + camYRef.current,          // smoothed, or the horizon bobs with every tick
      ...
      // the engine note follows your speed
      sfx.updateEngine?.(Math.abs(me.speed) / 26, !!me.airborne);
      ...
      if (q?.length) {
        ...
        for (const ev of q.splice(0)) {
          ...
          // ---- THE MODES ----
            ...
            if (ev.reason === "last_standing") pushFeed(`👑 ${nameOf(ev.winner)} is the last kart rolling!`);
          }
  ...
  return (
    <>
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{ transform: "translateY(-120px)", textAlign: "center" }}>
              <div style={{ fontSize: compact ? 26 : 34 }}>{modeYou.carrying ? "🏠" : "🚩"}</div>
          ...
          {modeYou.isDrawer && (
            <div style={{
              ...
              <div className="dim" style={{ fontSize: 11, marginTop: 4 }}>
                HOLD SPACE to pour · 1–5 to stamp · stay inside the rope
              </div>
            </div>
          )}
          {/* everyone's options */}
      ...
      <div data-qa="hud-chips" style={{ position: "absolute", left: 16, top: 14, display: "flex", flexDirection: "column", gap: 6, alignItems: "flex-start", pointerEvents: "none" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
        <div style={{ ...chip, fontSize: compact ? 19 : 30 }}>LAP {hud.lap}/{hud.laps}</div>
      ...
      <div data-qa="hud-controls" style={{ position: "absolute", left: 16, bottom: 16, display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <button className="btn" style={{ fontSize: compact ? 14 : 19, padding: "8px 16px" }} onClick={onLeave}>{t("play.hud.returnToLobby")}</button>
      </div>
    </>
  );
}
