    ...
    function frame(now) {
      ...
      // The last stretch settles behind your kart for the light sequence.
      {
        ...
        if (fl > FLY_END) {
          ...
          const k = 1 - (fl - FLY_END) / total;          // 0 -> 1 across the flight
          const n = track.samples.length;
          // sweep most of a lap, easing out, riding a little above the road
          const idx = Math.floor(k * n * 0.92) % n;
          const s = track.at(idx);
          const look = track.at((idx + 26) % n);
          const lift = 9 + Math.sin(k * Math.PI) * 7;    // rise, then settle
          const side = Math.sin(k * Math.PI * 2) * 6;
          cam.position.set(s.x + (-s.tz) * side, (s.y || 0) + lift, s.z + (s.tx) * side);
          cam.lookAt(look.x, (look.y || 0) + 1.2, look.z);
