...
import { __cosSelfTest, buildCar, animateCar } from "../game/carMesh.js";
...
export default function Race3D({ view, roomId, conn, inputLocked, onLeave, eventQueue }) {
  ...
  const trackRef = useRef(null);        // the built track, for the slope-pitch sampler
  const threatPrevRef = useRef(new Map()); // entity id → last position, for closing-speed
  const threatUntilRef = useRef(0);        // warning stays lit until this timestamp
  ...
  useEffect(() => {
    ...
    if (arenaDef) {
      ...
      window.__cosTest = __cosSelfTest;   // QA: run every cosmetic builder
      initEnvironment(renderer, scene);
    }
    ...
    // (trackRef is filled after buildWorld; the pitch sampler reads it)
  ...
  const sameKeys = (a, b) => a && b && KEY_FIELDS.every((k) => !!a[k] === !!b[k]);
    ...
    function noteSnapshots() {
      ...
      for (const p of v.players) {
        ...
        rec.immUntil = (p.imm || 0) > 0 ? tNow + p.imm : 0;
        rec.info = p;
        if (p.id === v.you?.id) {
    ...
    sfx.startEngine?.();
    // premium gear retunes the engine: your kart SOUNDS like what you equipped
    ...
    function sampleHud(tNow) {
      ...
      setHud({
        ...
        imm: my?.imm ?? 0,
        threat: performance.now() < threatUntilRef.current,
        kited: !!my?.kited,
    ...
    function frame(now) {
      ...
      for (const [id, rec] of meshes) {
        ...
        // ---- SLOPE PITCH ----
        // The kart was always rendered LEVEL: parked on the overpass descent it
        // hovered on an invisible flat. Sample the road's grade at the kart and
        // tilt the whole mesh to match — order YZX so pitch happens around the
        // kart's own axle line after the yaw.
        {
          if (rec.mesh.rotation.order !== "YZX") rec.mesh.rotation.order = "YZX";
          const S = trackRef.current?.samples;
          if (S && S.length) {
            let hi = rec.hintI ?? 0, hd = Infinity;
            for (let o = -10; o <= 10; o++) {
              const j = ((hi + o) % S.length + S.length) % S.length;
              const q = S[j];
              const d = (q.x - x) * (q.x - x) + (q.z - z) * (q.z - z);
              if (d < hd) { hd = d; rec.hintI = j; }
            }
            const N = S.length, j = rec.hintI;
            const a = S[((j - 2) % N + N) % N], b = S[(j + 2) % N];
            if (!a.gap && !b.gap) {
              const run = Math.hypot(b.x - a.x, b.z - a.z) || 1;
              const tx2 = (b.x - a.x) / run, tz2 = (b.z - a.z) / run;
              const dirDot = Math.cos(heading) * tx2 + Math.sin(heading) * tz2;
              const target = Math.atan2(((b.y || 0) - (a.y || 0)) * Math.sign(dirDot || 1), run) ;
              rec.pitchVis = (rec.pitchVis || 0) + (Math.max(-0.45, Math.min(0.45, target)) - (rec.pitchVis || 0)) * Math.min(1, dt * 9);
              rec.mesh.rotation.z = rec.pitchVis;
            }
          }
        }
        // ---- IMMUNITY BLINK ----
        if (rec.immUntil && tNow < rec.immUntil) {
          rec.mesh.visible = Math.floor(tNow * 9) % 2 === 0;
          rec._blinked = true;
        } else if (rec._blinked) {
          rec.mesh.visible = true;
          rec._blinked = false;
        }
      ...
      // ---- INCOMING THREAT ----
      // Track hostile entities frame-to-frame; anything behind you, inside 30m
      // and CLOSING lights the warning so a hit is never invisible.
      {
        const ents = v?.entities || [];
        const prev = threatPrevRef.current;
        const myX = me.x, myZ = me.z, hx = Math.cos(me.heading || 0), hz = Math.sin(me.heading || 0);
        let hot = false;
        for (const e of ents) {
          if (!e || !["balloon", "squirt", "bouncer", "wave"].includes(e.kind)) continue;
          if (e.by && v?.you?.id && e.by === v.you.id) continue;   // your own shot
          const dx = myX - e.x, dz = myZ - e.z;
          const dist = Math.hypot(dx, dz);
          const p0 = prev.get(e.id);
          prev.set(e.id, { x: e.x, z: e.z });
          if (dist > 30 || !p0) continue;
          const vx = (e.x - p0.x), vz = (e.z - p0.z);
          const closing = (vx * dx + vz * dz) / Math.max(0.001, dist);
          const behind = (dx * hx + dz * hz) > dist * 0.25;   // it's aft of you
          if (closing > 0.12 && behind) { hot = true; break; }
        }
        if (hot) threatUntilRef.current = performance.now() + 750;
        for (const [k2] of prev) if (!ents.find((e) => e && e.id === k2)) prev.delete(k2);
      }
      ...
      {
        const lo = v?.you?.loadout || {};
        const prof = lo.shoes === "shoes_comet" ? "hyper"
          : lo.body === "body_regalia" ? "vtwin"
          : lo.oxygenTank === "tank_jetpack" ? "jet"
          : lo.shoes === "shoes_neon" ? "electric" : "standard";
        if (prof !== window.__engProf) { window.__engProf = prof; sfx.setEngineProfile?.(prof); }
      }
      ...
      if (q?.length) {
        ...
        for (const ev of q.splice(0)) {
          ...
          } else if (ev.type === "reset_blocked") {
            if (ev.playerId === myId) pushFeed("🛑 STOP THE CAR to use the shovel (R)");
          }
    ...
    <>
      ...
      {hud.threat && (
        <div style={{
          ...
          background: "rgba(226,60,50,0.92)", color: "#fff", fontWeight: 900,
          padding: "5px 14px", borderRadius: 10, letterSpacing: 1, fontSize: 13,
          animation: "pulse 0.6s ease-in-out infinite alternate", pointerEvents: "none",
        }}>⚠ INCOMING · SHIFT = LOOK BACK</div>
      )}
      <div data-qa="hud-erosion" style={{ position: "absolute", right: 18, bottom: compact ? 82 : 108, display: "flex", gap: 5, alignItems: "center", pointerEvents: "none" }}>
        ...
        {(hud.imm ?? 0) > 0 && (
          <span style={{
            ...
            background: "rgba(47,230,200,0.9)", color: "#08302a",
