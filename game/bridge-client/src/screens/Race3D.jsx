...
export default function Race3D({ view, roomId, conn, inputLocked, onLeave, eventQueue }) {
  ...
  useEffect(() => {
    ...
    const bloom = new UnrealBloomPass(new THREE.Vector2(1, 1), 0.35, 0.45, 1.05);
    ...
    function frame(now) {
      ...
      for (const [id, rec] of meshes) {
        ...
        if (rec.isMe && me.seeded) {
          ...
          rec.yVis = (rec.snapA.y ?? 0) + ((rec.snapB.y ?? 0) - (rec.snapA.y ?? 0)) * f;
        ...
        rec.mesh.userData.baseY = rec.yVis || 0;   // animateCar owns position.y
...
function HowToPlay({ modeId, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      background: "rgba(6,10,16,0.72)", cursor: "pointer",
    }}>
      <div onClick={(e) => e.stopPropagation()} style={{
        width: "min(560px, 92vw)", maxHeight: "86vh", overflowY: "auto",
        background: "linear-gradient(180deg, #14202b, #0c151d)",
        border: "3px solid var(--volt, #2fe6c8)", borderRadius: 16, padding: "26px 30px",
        boxShadow: "0 24px 80px rgba(0,0,0,0.6)", cursor: "default", textAlign: "left",
      }}>
        <div className="impactf" style={{ fontSize: 12, letterSpacing: "0.18em", color: "var(--volt, #2fe6c8)" }}>HOW TO PLAY</div>
        <div className="display" style={{ fontSize: 40, color: "#fff", lineHeight: 1, margin: "2px 0 6px" }}>{info.title}</div>
        <div style={{ fontSize: 16, fontWeight: 800, color: "#ffd479", marginBottom: 14 }}>{info.goal}</div>
        <ul style={{ margin: "0 0 18px", padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 8 }}>
          {info.rules.map((r, i) => (
            <li key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", color: "#dfe9f0", fontSize: 14 }}>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.12)", paddingTop: 14 }}>
          <div className="impactf" style={{ fontSize: 11, letterSpacing: "0.14em", color: "var(--dim, #8aa)", marginBottom: 8 }}>CONTROLS</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 18px", fontSize: 13, color: "#cdd8e0" }}>
        ...
        <button onClick={onClose} className="btn btn-hot" style={{ marginTop: 20, width: "100%", padding: "12px", fontSize: 15, fontWeight: 900 }}>
...
function RaceHud({ hud, toast, onLeave, posFlash, wrongWay, roulette, miniRef, lapClock, feed = [],
  ...
  return (
    <>
      ...
      {/* DERBY — lives, the closing ring, and who's left */}
      {modeId === "derby" && modeYou && (
        <div data-qa="hud-derby" style={{ position: "absolute", top: 14, right: 18, textAlign: "right", pointerEvents: "none", zIndex: 14 }}>
          <div className="impactf" style={{ fontSize: 11, letterSpacing: "0.14em", color: "var(--dim)" }}>LIVES</div>
          <div style={{ fontSize: compact ? 26 : 36 }}>
            {"💚".repeat(Math.max(0, modeYou.lives))}{"🖤".repeat(Math.max(0, 3 - modeYou.lives))}
          </div>
          <div className="impactf" style={{ fontSize: 11, color: "var(--hot)", marginTop: 6 }}>
      ...
      {modeId === "pearl" && modeYou && (
        <div data-qa="hud-pearl" style={{ position: "absolute", inset: 0, pointerEvents: "none", zIndex: 14 }}>
          <div style={{ position: "absolute", top: 14, right: 18, textAlign: "right" }}>
            ...
            <div className="display" style={{ fontSize: compact ? 30 : 44, color: "#fff1f5" }}>
      ...
      <div data-qa="hud-standings" style={{ position: "absolute", right: 16, top: 14, ...chip, padding: "10px 12px", pointerEvents: "none" }}>
        {hud.standings.map((s) => (
          <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: compact ? 15 : 21, opacity: s.me ? 1 : 0.85, fontWeight: s.me ? 900 : 700 }}>
      ...
      <div data-qa="hud-controls" style={{ position: "absolute", left: 16, bottom: 16, display: "flex", gap: 8, alignItems: "center" }}>
        {!compact && <div style={{ ...chip, fontSize: 18, opacity: 0.92, pointerEvents: "none" }}>
          WASD / arrows · SPACE = use item · SHIFT = look back · R = shovel reset · ESC = menu
        </div>}
      </div>
    </>
  );
