...
export function PlayerActions({
  ...
  onKarma, onFriend, size = "sm", isBot = false,
}) {
  const { t } = useI18n();
  ...
  // Bots have no account, but karma to a bot still counts (giver-side record +
  // achievements + testing) — so a bot target renders the karma button and
  // nothing else. Friend/report need a real account.
  ...
  return (
    <span className="row gap-s" style={{ alignItems: "center", flexWrap: "wrap" }}>
      ...
      {report && !isBot && <ReportModal name={name} userId={userId} matchId={matchId} onClose={() => setReport(false)} />}
    </span>
  );
}
