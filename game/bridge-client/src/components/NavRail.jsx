      ...
      <div style={{ textAlign: "center", marginTop: 8 }}>
        {profile && (
          <div style={lvlChip} title={`${profile.xp} XP`}>
            <div style={{ fontSize: 8, letterSpacing: "0.15em", color: "#7a5c1e", fontWeight: 800 }}>LV</div>
            <div className="display" style={{ fontSize: 26, color: "#4a3722", lineHeight: 0.9 }}>{profile.level}</div>
          </div>
        )}
        ...
        {/* The build stamp. There was no version visible anywhere — you could deploy
            and have no way of knowing whether the thing on screen was the thing you
            pushed. */}
        <div style={{ fontSize: 8, color: "var(--faint)", marginTop: 6, letterSpacing: "0.04em" }}
             title={`built ${typeof __BUILD_TIME__ !== "undefined" ? __BUILD_TIME__ : "?"}`}>
...
// The level chip. It was a gold BORDER over an almost-transparent gold tint, with
// gold text — which worked when the nav was near-black and is invisible on white.
// Solid gold now, with dark text on it: it reads as a little enamel badge.
const lvlChip = {
  ...
  color: "#4a3722",
  borderRadius: 10,
  boxShadow: "0 3px 0 #c47f14, 0 6px 12px rgba(120,90,50,0.24)",
};
