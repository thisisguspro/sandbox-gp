...
export function createGameConnection({ onState, onEvents, onError, onConnect, onDisconnect }) {
  ...
  socket.on("state", (msg) => {
    ...
    if (v && typeof v === "object") {
      ...
      // B7: ready-up snapshot for the lobby
      if (msg.ready) v.ready = msg.ready;
      if (typeof msg.allReady === "boolean") v.allReady = msg.allReady;
    }
    onState && onState(v);
  ...
  return {
    ...
    setReady: (roomId, ready) => socket.emit("set_ready", { roomId, ready }),
    listOpenGames: () => new Promise((r) => socket.emit("list_open_games", {}, cb(r))),
    quickJoin: (name, mapId) => new Promise((r) => socket.emit("quick_join", { name, mapId, token: token() }, cb(r))),
    quickRace: (name) => new Promise((r) => socket.emit("quick_race", { name, token: token() }, cb(r))),
