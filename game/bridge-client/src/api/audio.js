...
// ---- ENGINE PROFILES ----
// Premium karts don't just look different — they SOUND different. A profile
// sets the oscillator characters and the pitch band; setEngineProfile can be
// called any time (mid-race included) and retunes the live graph.
const ENGINE_PROFILES = {
  standard: { w1: "sawtooth", w2: "sawtooth", det: 1.012, base: 52, span: 96,  q: 3, lp: 300, lpSpan: 1500 },
  vtwin:    { w1: "square",   w2: "sawtooth", det: 1.006, base: 38, span: 70,  q: 6, lp: 220, lpSpan: 1100 },   // Golden Hour: deep, lazy thunder
  hyper:    { w1: "sawtooth", w2: "sawtooth", det: 1.03,  base: 84, span: 150, q: 8, lp: 420, lpSpan: 2600 },   // Comet Treads: a race whine
  electric: { w1: "triangle", w2: "sine",     det: 1.5,   base: 120, span: 140, q: 2, lp: 600, lpSpan: 2000 },  // Neon Rims: an EV hum
  jet:      { w1: "sawtooth", w2: "square",   det: 1.002, base: 66, span: 120, q: 5, lp: 380, lpSpan: 2200 },   // Twin-Jet Pack
};
let _profile = ENGINE_PROFILES.standard;
export function setEngineProfile(name) {
  _profile = ENGINE_PROFILES[name] || ENGINE_PROFILES.standard;
  if (_engine) {
    try {
      _engine.osc1.type = _profile.w1;
      _engine.osc2.type = _profile.w2;
      _engine.filter.Q.value = _profile.q;
    } catch {}
  }
}

export function startEngine() {
  // Make sure we have a LIVE context. If the page hasn't had a user gesture yet
  // the context can be suspended, and oscillators started on a suspended context
  // come up silent — which is exactly why the engine was inaudible. initAudio()
  // creates + resumes it.
  if (!ctx) initAudio();
  ...
  try {
    ...
    osc1.type = _profile.w1;
    osc2.type = _profile.w2;
    osc1.frequency.value = _profile.base;
    osc2.frequency.value = _profile.base * _profile.det;   // detuned — the BEAT between them is the engine

    filter.type = "lowpass";
    filter.frequency.value = _profile.lp;
    filter.Q.value = _profile.q;

    // Starts at the idle bed so the kart is audibly alive the moment the race
    // begins; updateEngine drives it from there.
...
export function updateEngine(speed01, airborne = false, throttle = 0) {
  if (!_engine || !ctx) return;
  const t = ctx.currentTime;
  const s = Math.max(0, Math.min(1, speed01));
  // How hard the gas is down, independent of how fast you're actually going —
  // this is what lets you HEAR yourself hit the gas even from a standstill
  // (stationary against a wall, spinning up on the line). |throttle| so reverse
  // revs too.
  const gas = Math.max(0, Math.min(1, Math.abs(throttle)));
  // rev = the audible driver. Mostly speed, but the gas alone lifts it a good
  // chunk so flooring it from a stop is instantly audible.
  const rev = Math.max(0, Math.min(1, s * 0.7 + gas * 0.45));

  // pitch rises with rev — an idle thrum up to a working note, with the gas
  // audibly kicking it up even before the kart has built speed
  const base = _profile.base + rev * _profile.span;
  _engine.osc1.frequency.setTargetAtTime(base, t, 0.05);
  _engine.osc2.frequency.setTargetAtTime(base * _profile.det, t, 0.05);

  // the filter OPENS as you accelerate: that's what makes it sound like effort
  _engine.filter.frequency.setTargetAtTime(_profile.lp + rev * _profile.lpSpan, t, 0.07);

  // and the volume. Now actually AUDIBLE — a real engine you hear on the gas,
  // not a whisper you notice only when it's gone. There's always an idle bed so
  // the kart is never dead-silent, and flooring it roughly triples it.
  const idle = 0.045;
  const target = _muted ? 0 : (idle + rev * 0.11) * (airborne ? 0.5 : 1);
  _engine.gain.gain.setTargetAtTime(target, t, 0.08);
}
