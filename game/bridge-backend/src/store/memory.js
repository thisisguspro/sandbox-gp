...
async getProfile(userId) {
  ...
  async giveKarma(giverId, { matchId, targetId } = {}) {
    ...
    // BOT KARMA: bots have no account, but karma to a bot still counts for the
    // GIVER — it records in karmaGiven (cap 2/match, same as humans), feeds the
    // giver's achievement/testing loop, and simply skips the receiver-side
    // bookkeeping. The match itself must still be one the giver recently played.
    if (!target) {
      const entryB = (giver.matchHistory || []).slice(0, 10).find((h) => h.matchId && h.matchId === matchId);
      if (!entryB) throw new Error("you can only give karma for your recent matches");
      if (!giver.karmaGiven || typeof giver.karmaGiven !== "object") giver.karmaGiven = {};
      const givenB = giver.karmaGiven[matchId] || (giver.karmaGiven[matchId] = []);
      if (givenB.includes(targetId)) throw new Error("you already gave karma to that pilot");
      if (givenB.length >= 2) throw new Error("karma cap reached for this match (2)");
      givenB.push(targetId);
      return { ok: true, matchId, targetId, bot: true, given: [...givenB] };
    }
  ...
  async equipCosmetic(userId, cosmeticId) {
    ...
    // BRIDGE THE TWO IDENTITY SYSTEMS. The nav rail, profile card and racer
    // card all read the legacy selectedAvatar/selectedBorder fields, while the
    // Locker writes the loadout — equipping an avatar or border in the Locker
    // silently changed nothing on screen. Equip now writes both.
    if (cos.slot === "avatar") u.selectedAvatar = cosmeticId;
    if (cos.slot === "border") u.selectedBorder = cosmeticId;
