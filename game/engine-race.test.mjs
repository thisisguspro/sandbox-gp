...
// ---- FIVE CIRCUITS, and RACE + TIME ATTACK always share them ----
{
  (RaceEngine.CIRCUITS.length === 5)
    ? ok(`five circuits: ${RaceEngine.CIRCUITS.join(", ")}`)
    : no(`expected 5 circuits, got ${RaceEngine.CIRCUITS.length}`);
  ...
  // every circuit must be genuinely its own place, not a recolour
  const themes = new Set(RaceEngine.CIRCUITS.map((id) => makeTrack(id).def.theme || "beach"));
  (themes.size === 5)
    ? ok(`all five circuits have their own theme (${[...themes].join(", ")})`)
    : no(`only ${themes.size} distinct themes across 5 maps — some are recolours`);
}
...
// ---- THE RIDE MUST BE SMOOTH: NO VERTICAL SLAPPING ----
{
  // Gustavo reported the ramps feeling like a staircase FOUR TIMES. I rewrote the
  // ground-height function three times — nearest-sample, linear, smoothstep,
  // grade-following — and every single time the worst vertical acceleration came out
  // at EXACTLY 48.4 m/s². Four different algorithms, one identical number.
  //
  // That is not a coincidence. It meant nothing I did upstream mattered, because the
  // FLOOR CLAMP at the very end of stepCar was snapping the car straight back onto
  // the raw sample height: `const floor = fsmp.y || 0`.
  //
  // The samples on the climb are 0.72m apart; the car covers 0.80m per tick. So it
  // passes more than one sample per tick, `nearest()` advances by one index some
  // ticks and two the next, and the clamp slapped the car up by a whole step each
  // time. The trace is unambiguous:
  //
  //     tick | hint | jump | dy
  //       13 |  651 |   1  | 0.054
  ...
  //
  // This test measures what a PLAYER actually feels: not the height (which was always
  // "continuous"), and not the velocity, but the vertical ACCELERATION — the jerk.
  // Gravity is 9.8 m/s². A car being slapped at 37 is being thrown around.
  for (const id of RaceEngine.CIRCUITS) {
    const t = makeTrack(id);

    // find the steepest part of this circuit and drive it at race pace
    let hi = 0, best = 0;
    for (let i = 0; i < t.samples.length; i++) {
      if ((t.samples[i].y || 0) > hi) { hi = t.samples[i].y || 0; best = i; }
    }
    if (hi < 1) { ok(`${id}: flat circuit, nothing to judder`); continue; }

    const start = Math.max(0, best - 90);
    const s0 = t.at(start);
    const st = {
      x: s0.x, z: s0.z, heading: Math.atan2(s0.tz, s0.tx),
      speed: 24, sampleHint: start, y: s0.y || 0,
    };

    const ys = [];
    for (let k = 0; k < 200; k++) {
      // steer down the centreline
      let j = st.sampleHint, left = 14;
      while (left > 0) {
        const a = t.at(j), b = t.at(j + 1);
        left -= Math.hypot(b.x - a.x, b.z - a.z);
        j++;
      }
      const tg = t.at(j);
      let d = Math.atan2(tg.z - st.z, tg.x - st.x) - st.heading;
      while (d > Math.PI) d -= 2 * Math.PI;
      while (d < -Math.PI) d += 2 * Math.PI;
      stepCar(st, { throttle: 1, steer: Math.max(-1, Math.min(1, d * 2.2)) }, DT, t, {});
      ys.push(st.y || 0);
    }

    const vel = [];
    for (let i = 1; i < ys.length; i++) vel.push((ys[i] - ys[i - 1]) / DT);
    const acc = [];
    for (let i = 1; i < vel.length; i++) acc.push(Math.abs(vel[i] - vel[i - 1]) / DT);
    acc.sort((a, b) => a - b);
    const p95 = acc[Math.floor(acc.length * 0.95)] || 0;

    // Gravity is 9.8. A ramp you DRIVE up should sit well under it most of the time;
    // a p95 above ~15 means the car is being thrown, and that reads as a staircase.
    (p95 < 15)
      ? ok(`${id}: the ride is smooth (p95 vertical jerk ${p95.toFixed(1)} m/s²)`)
      : no(`${id}: the car is being SLAPPED up the ramp — p95 vertical acceleration is ${p95.toFixed(0)} m/s², ${(p95 / 9.8).toFixed(1)}x gravity. That is the staircase.`);
  }
}
