...
await fetch(`${BASE}/internal/match-result`, {
  method: "POST", headers: { "content-type": "application/json", "x-service-key": KEY },
  body: JSON.stringify({ matchId: `tt_${Date.now()}`, winner: uid, mode: "timeattack", participants: [{ ...participants[0], bestLapSec: 13.37, totalSec: 44.4 }] }),
}).then(j);
...
await fetch(`${BASE}/internal/match-result`, {
  method: "POST", headers: { "content-type": "application/json", "x-service-key": KEY },
  body: JSON.stringify({ matchId: `tt2_${Date.now()}`, winner: uid, mode: "timeattack", participants: [{ ...participants[0], bestLapSec: 15.0 }] }),
}).then(j);
await fetch(`${BASE}/internal/match-result`, {
  method: "POST", headers: { "content-type": "application/json", "x-service-key": KEY },
  body: JSON.stringify({ matchId: `tt3_${Date.now()}`, winner: uid, mode: "timeattack", participants: [{ ...participants[0], bestLapSec: 12.9 }] }),
}).then(j);
...
await fetch(`${BASE}/internal/match-result`, {
  method: "POST", headers: { "content-type": "application/json", "x-service-key": KEY },
  body: JSON.stringify({ matchId: `e3_${Date.now()}`, mode: "timeattack", laps: 3, participants: [{ userId: g2.user.id, name: "EconKid", won: true, place: 1, bestLapSec: 20, totalSec: 66 }] }),
}).then(j);
