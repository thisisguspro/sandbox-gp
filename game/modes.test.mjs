...
function play(mode, { seconds = 500, players = null } = {}) {
  const e = new RaceEngine({ config: { seed: 5, mode } });
...
// ---- 3. SAND ARTIST ----
{
  ...
  // a wrong guess must actually destroy you
  ((seen.guess_wrong || 0) > 0)
    ? ok(`sand artist: a wrong guess blows you up (${seen.guess_wrong / 4})`)
    : no("sand artist: nobody ever guessed wrong");
}
