...
// ---- racing is UNTOUCHED ----
{
  ...
  // Grand Prix is now 4 min / 8 max (matching every arena mode); the only solo
  // mode is Time Attack. A plain race still has no mode rules and no arena.
  (e.rules === null && e.arena === null && e.map.maxPlayers === 8 && e.map.minPlayers === 4)
    ? ok("a race is still a race: no mode rules, no arena, 4–8 players")
