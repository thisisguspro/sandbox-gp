...
async function main() {
  ...
  // ---- TIME TRIAL over sockets: no auto-fill, no items, mode in view + result ----
  {
    ...
    const room3 = await new Promise((r) => s3.emit("create_room", { config: { isPublic: false, mode: "timeattack", items: false, autoFill: false, laps: 1, finishTimeoutSec: 4 }, name: "ClockHost" }, r));
    if (!room3?.roomId) { no("timetrial create_room failed"); }
      ...
      // `mode` is the engine's mode OBJECT now ({ id, label }) — there used to be a
      // second, bare-string `mode` key in the same view literal that silently
      // overwrote it, which is what made four modes unselectable in the lobby.
      const m3 = typeof v3?.mode === "string" ? v3.mode : v3?.mode?.id;
      (m3 === "timeattack") ? ok("time attack: view carries the mode") : no(`view mode: ${m3}`);
