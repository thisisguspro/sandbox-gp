...
## v24.3.0 — I finally found the staircase

### The bug I chased four times

Gustavo reported the ramps feeling like a staircase **four separate times**. I
rewrote the ground-height function **three times** — nearest-sample, then linear
interpolation, then smoothstep, then grade-following — and every single time the
worst vertical acceleration came out at **EXACTLY 48.4 m/s².**

Four different algorithms. One identical number. That is not a coincidence: it meant
**nothing I did upstream mattered.**

The culprit was the **floor clamp at the very end of `stepCar`**:

```js
const floor = fsmp.y || 0;   // the RAW sample height
```

However smoothly you compute the ground, if the last thing you do is **clamp to a
quantised height, you have a quantised height.**

And here is why that mattered so much. The samples on the climb are **0.72 metres
apart**. At race pace the car covers **0.80 metres per tick.** So it passes *more
than one sample per tick*, `nearest()` advances by one index some ticks and two the
next, and the clamp slapped the car up by a whole step each time. The trace is
unambiguous:

```
tick | hint | jump | dy
  13 |  651 |   1  | 0.054
  14 |  653 |   2  | 0.106     <-- DOUBLE
  15 |  654 |   1  | 0.052
```

**The car wasn't stepping. It was being slapped, once per sample, at five times
gravity.**

| | Before | After |
|---|---|---|
| median vertical jerk | 2.4 m/s² | **1.2** |
| **p95** | **37.0 m/s²** | **7.0** |
| worst | 48.4 m/s² | 32 (float noise on flat road) |

Every circuit now measures **3.6 – 5.8 m/s²** at p95, all well under gravity. There's
a test that fails if any circuit exceeds 15.

### The camera was underground

That screenshot — terrain slicing across mid-screen with **sky visible underneath
it** — was the camera buried in the sand.

The camera sits **4.9 units behind** the car, but its height was derived from the
**car's** altitude. On a slope, the ground five metres behind you is at a completely
different height from the ground under you. On a descent it's HIGHER — so the camera
drove straight into the hill.

It samples the ground **at its own position** now, and can never sit below it.
Measured live over a full lap: **zero underground frames, minimum clearance 2.22m.**

### The rocks weren't scenery — they were worse

The `rockfall` hazard sat at **lateral 0.0 — dead centre of the racing line.** There
was no line through it; you simply hit it. And it was rendered as **brown
dodecahedrons**: the exact same shape and colour as the roadside decoration rocks.

So a deliberate obstacle read as *"somebody left rocks on the track."* A hazard you
can't distinguish from scenery isn't a hazard, it's a trap.

Now: off the centreline (verified — **every solid hazard leaves a 2.5m+ gap** for a
1.8m kart), dark volcanic stone, **warning paint** on the road, hazard chevrons, and
dust still settling.

### The podium

Rotated, and the place numbers are now built on **both faces** — I got its facing
wrong twice by reasoning about geometry I cannot see, so building the numbers on both
sides removes the entire class of bug.

## v24.2.0 — the staircase, the floating ball, the lobby, and the buy button

### The hillside was a staircase

The embankment was **one flat quad per road segment**, and three things went wrong
together:

* `if (Math.abs(y) < 0.15) continue` — the skirt **appeared and vanished**, leaving
  a hard step at every boundary between flat road and climbing road.
* A single quad from kerb to sand is a **ramp with a sharp crease** at each end.
  Real ground eases out of the flat and eases back into it.
...
That's the terracing in the screenshot. It's a proper **grid** now: it runs the whole
lap with no gaps to step over, it has six rows across the slope with a **smoothstep**
profile, and the **vertices are shared** between neighbouring cells so the normals
average and the whole thing shades as one continuous surface.

Verified on every map: the worst 1-metre step is **0.57m**, on the tallest crest in
the game.

### The floating beach ball

The scenery was planted using a **linear** ramp from road-height down to zero. But
the ground uses a **smoothstep** — it has to, or the hillside creases. So things were
standing on a straight line while the ground **curved away beneath them**, and near
the top of the slope, where smoothstep is flattest, the gap was metres.

There is exactly **one formula** for the surface. Everything that stands on it now
uses that formula. Verified: **zero gap, on every map, for every piece.**

### The lobby couldn't scroll

`body { overflow: hidden }` **and** every screen's container was
`{ height: "100%", overflow: "hidden" }`. So on any viewport shorter than the content
— a laptop, a small window — the bottom of the page was simply **cut off**, with no
scrollbar and no way to reach it. You had to zoom the browser out.

At 1280x600 there was **259px below the fold and no way to get to it**. The screens
scroll now.

### The buy button was buried

The box card rendered `box.odds` — **a field that does not exist**. The store sends
`drops`, with a `weight`. So the drop table never rendered at all, and the card was
built around data it never had.

Now it shows the **odds by rarity** (which is what you actually decide on), the full
84-item list is **one click away and scrolls in its own box**, and the **OPEN button
is always visible**:

> Treasure Chest · **Common 60.6% · Rare 30.7% · Epic 8.7%** · *See all 84 items* ·
> **OPEN · 700 SEA GLASS**

Card height: **1000+px → 349px.**

## v24.1.0 — everything from the last screenshot, and the material that fixes it all

### The one-line change that lifts every object in the game

**Everything was `MeshLambertMaterial`.** Lambert has **no specular term at all** —
it literally cannot catch the light. That is why every item, however much geometry
I bolted onto it, still read as "shapes slapped together": they were shapes made of
**matte cardboard**.

A beach toy is injection-moulded plastic. It's **shiny**. That hot little highlight
where the sun hits it is what tells your eye the object is solid, curved and real —
more than any amount of extra geometry ever will.

Phong, everywhere. **1,587 shiny meshes.** One line, and the whole game stops
looking like paper craft.

### Rotations

* **The boost flame was on the SIDE of the car.** The streaks were positioned at
  `x = -1.6`, but the car's forward is **+X** — so they trailed off the flank. And
  they were just... lines.
* **The flag hung sideways.** A `PlaneGeometry`'s default face is the XY plane, so
  the pennant lay flat to the world instead of streaming behind the kart. And the
  animation **rotated it about Y** — spinning it like a weathervane, which is not a
  thing a flag does.
* **The crowd stands faced north-south on an east-west straight** — a textbook
  90-degree error. The rotation used `atan2(-tx, tz)` where it needed
  `atan2(-tz, tx)`. Verified: front-to-track alignment is now **1.00** at every
  point on every circuit.

### The floating trees

Scenery was planted at **`p.y` — the ROAD's height**. But it isn't on the road; it's
out on the beach, and the beach is a **flat plane at y = 0**. So beside the 8.9m
climb, a palm forty metres out was hanging **nine metres in the air**. Every piece
now sits on the surface it's actually standing on (the embankment slope, or flat
sand). Verified across all six maps: **zero floaters**.

### The ramp "acting like stairs"
...
A real road is 6–10%. A steep mountain pass is 12%. A kart hitting a **44% wall** at
24 m/s climbs **0.2 metres per tick** — and *that* is what a staircase feels like.
Every crest is still exactly as high; they're just spread over enough road to drive
up. **Sandcastle 44% → 16%. Dunes 32% → 10%.**

### The layer covering the beach

Every screen had `const wrap = { background: "radial-gradient(120% 100% at 20% 0%,
#211726 0%, var(--ink) 55%)" }` — a **dark purple** fading to an **opaque cream**.
On a bright summer beach game. Plus the app shell had a **solid cream fill** across
the whole viewport. I drew an illustrated beach and then painted over every pixel
of it. Both gone; the sky, sea and palms are visible now.

### Anime

* **BOOST** — twin exhaust flames out the **back** with white-hot cores and blue
  roots, a **shockwave ring** pulsing off the tail, and ten **speed lines** raking
  past. All animated.
* **STUNNED** — three **birds circling your head**, flapping, with **stars** orbiting
  the other way. The oldest gag in animation, and it tells the entire grid, instantly
  and without a word, that the kart in front of you is helpless. They **chirp**, and
  there's a thud when you get walloped.

### Flags

A new slot, and **11 flags** across all nine sets. And a flag is not a coloured
rectangle — it has a **device** on it, and that device is the whole reason you'd
want one: the **Jolly Roger** has a skull with eye sockets and crossed bones; the
**Chequered Flag** has squares; **Storm Warning** has a lightning bolt.

### Sound

* **The lobby beep was a SQUARE WAVE at 520Hz** — a buzzer, sitting right in the
  ear's most sensitive band, firing repeatedly while you wait. Now a soft sine at
  392Hz, half the volume.
* **A subtle ENGINE.** Two detuned sawtooths (the beat between them *is* the engine),
  a low-pass that opens as you accelerate, and a gain that never rises above a
  whisper. Its pitch follows your speed, so you can hear yourself accelerate.

### And a bug the effects exposed

`syncStatuses` had **no clock**. Both the boost and the stun animations used `t`,
which was **never defined** — a ReferenceError every frame the moment anyone boosted
or got hit.

### Performance

The detail cost real frames: **3,051 meshes, 816 draw calls, 71ms.** The crowd alone
was 84 individual spheres. **InstancedMesh** for the crowd and all the roadside
dressing: **2,017 meshes, and a 19ms median frame** — with every bit of the shine
kept.

## v24.0.0 — the shop, the character, and the four from the screenshots

**Version bumped to 24.0.0, and it's now VISIBLE in the app** (bottom of the nav
rail). There was no version anywhere — you could push to Render and have no way of
knowing whether the thing on screen was the build you deployed.

### What you were actually selling

I measured the nine crown jewels — the real-money items — and **five of the nine
were one or two meshes**:

| Item | Price | Was |
|---|---|---|
| **Sun Halo** | 6 shells | **ONE TORUS.** |
| **Thunder Horns** | 6 shells | two cones |
| **Winged Helm** | 6 shells | two triangles |
| **Neon Visor** | 4 shells | a band and a pane |
| **Great White Float** | 4 shells | the **free starter torus** with a fin stuck on |

You cannot charge real money for a primitive. Rebuilt:

* **Sun Halo** → 17 parts. A white-hot core ring, a warm corona, a soft bloom,
  **eight tapering rays**, and motes of light drifting off it.
* **Thunder Horns** → 28 parts. **Curved, ridged** horns like a ram's, dark as wet
  slate, with **lightning arcing between them** and sparks flying off the tips.
* **Winged Helm** → 16 parts. A hard shell with **panel lines**, a mirrored visor
  with a glint, **swept thruster fins** with glowing intakes and exhaust bloom, and
  an antenna.
* **Neon Visor** → 15 parts. A wraparound lens with a **scanline laser grid**
  burning across it, a hot-pink brow light bar, chrome, and side pods with vents.
* **Great White Float** → 24 parts. Body, snout, **gills, teeth, an eye with a
  glint**, dorsal fin, pectorals, tail flukes — and the **valve**, because it's an
  inflatable.
* **Rescue Buoy** → 19 parts. A **torpedo hull** with a keel, six moulded grab
  handles, a reflective stripe, coiled rope, and a snap hook.
* **Coral Crown** → 43 parts. A nacre band with **branching coral** in three
  colours, pearls set into it, sea-fan fronds, and a big pearl at the front.
* **Pineapple Head** → 69 parts. A **diamond cross-hatched rind**, eleven splayed
  serrated leaves — and a **tiny paper umbrella** stuck in the side, because the
  funniest item in a shop is very often the best seller.

There's a test that fails if any premium item drops below 10 parts.

### The character was a snowman

The driver — the thing every costume in the shop hangs off, the thing you look at
for the whole race — was **SIX MESHES**: a capsule, a sphere head, a sphere helmet,
and two spheres for hands.

**Now 36 meshes.** A racing suit with a zip and a collar, **shoulders**, arms that
**actually reach the wheel**, gloves with cuffs and thumbs, a neck, a face with a
**jaw, a nose, eyes, pupils and brows**, ears — and a real racing helmet with a
rim, a crown stripe, a **rear spoiler** and a chin strap.

### And the four from the screenshots

**"The front of the car is backwards."** It wasn't backwards — it was a **4-segment
cylinder**, i.e. a pyramid, rotated 45°. From any normal angle it presents a flat
diamond face, so the front of the kart read as a red SLAB. A flat plate is what the
BACK of a vehicle looks like, so of course it looked reversed. It's a proper low
rounded wedge now, with a **splitter**, end plates, a bonnet and **headlights**.

**"The under part of the jumps glitches through the wall."** The road ribbon is
**one-sided**. Standing on the beach beneath an 8.9-metre climb and looking up, you
saw straight through it — that brown "ceiling" was the underbelly of a road that
has no underbelly. It has a **soffit, fascia and PILLARS** now: from below it reads
as a raised causeway you're driving under.

**"Hoops on the bridge load in the lower part."** The server sent gates as
`{x, z}` only, and `challengeView` **silently stripped `y`** on the way out. The
client then guessed the height with a search **pinned to ground level**. Gates carry
their altitude now — a hoop on the crest sits at 8.1m and descends with the road.

**"The sharp turn looks weird."** Two control-point clusters **doubled back on
themselves**: `(140,74)→(196,96)→(120,124)` and `(-96,-170)→(-72,-182)→(-120,-222)`.
Each folded the spline over its own kerbs in the space of two points, producing a
**105° "corner"**. They're sweeping arcs now — **worst bend 105° → 70°**.

**"The podium shows its back side."** The camera sits at +Z; a kart's forward is
+X. So the podium presented three karts **side-on with their tails toward you**. The
karts are turned to face the camera now.

**"The gradient background looks stupid."** It was three stacked CSS gradients
pretending to be a horizon — a smear with no clouds, no distance, no objects.
Replaced with an **illustrated SVG beach**: a high sun with a corona, layered anime
clouds, a real sea with a horizon line and a glitter path, headlands in the haze,
palm trees, a parasol, and the sand you're standing on. Sharp at any size, on every
screen except the in-game HUD.

## v23.20 — the wardrobe

### It was the diving game's wardrobe with beach names painted on

The slot keys were `oxygenTank`, `breather`, `weapon` and `bandana`. The item IDs
still read `breather_oni`, `breather_kitsune`, `tool_katana`, `belt_obi` — the
SAMURAI game's list. There was a `background` slot for a "vista behind the train"
that **nothing has ever rendered**, and a `victoryPose` slot that **no code
anywhere reads**.

And there were **FIFTEEN DUPLICATE NAMES**. "Kraken Snorkel" twice. "Surfboard"
twice. "Cat-Ear Helmet" twice. Two items, same name, different mesh.

**118 items and ZERO sets.** "Ronin Drifter Suit" beside "Kraken Snorkel" beside
"Molten Sun Regalia" — a bag of loose accessories with no story a player could
...
nobody has ever dreamed of being a "Superior Rare Turbocharger".

### Nine sets. Nine personas.

| Set | Who you are |
|---|---|
| **Lifeguard** | First on the sand, last off it. The one who saves everybody. |
| **Surf Punk** | Salt-crusted, sun-bleached, gloriously unbothered. |
| **Tiki King** | Carved masks, burning torches, and a very loud shirt. |
| **Neon Arcade** | Boardwalk at midnight. Chrome, hot pink, a cassette deck. |
| **Mecha Pilot** | Panel lines, a mirrored visor, thrusters you don't strictly need. |
| **Coral Court** | Deep-sea royalty. Pearl, nacre, a crown of living coral. |
| **Sandcastle** | A bucket, a spade, a paper crown. Utterly unserious. |
| **Storm Chaser** | A monsoon in kart form. Static in the air, a lightning rod. |
| **Golden Hour** | The last hour of light, and the longest shadows on the beach. |

**110 items, every one in a set, zero duplicate names.** The Locker shows your
progress through each collection — *9/11 Lifeguard* — which is the entire
monetisation loop and the one the research says works.

### The economy, fixed

**The premium store was selling items that don't exist** — `shoes_glow`,
`bg_snowpass`, `pose_meditate`, `border_neon` — across fifty-three hand-written
rows that had drifted out of sync with the catalogue. It's **generated** from the
catalogue now, so it can't drift again.

**One crown jewel per set** in the Shell store (4–6 shells). They cannot be
crafted, scrapped, or pulled from a chest — paying cash for a thing a rival can
melt down would be a joke. And **at most ONE cash-only piece per set**: every
collection is finishable free. A set you can only complete with money isn't a
collection, it's a hostage.

### Everything that was invisible

* **Every kart shell was the same car.** There were exactly two shells with any
  identity — a regex for "mecha" and one for "ronin". "Neon Speedster", "Tiki
  Cruiser" and "Storm Drifter" all came out in the same paint. Seven distinct
  machines now: rescue red, neon black, gunmetal, storm grey, carved wood, gold.
* **Every wheel was a hash of its own name.** `wheelStyle` knew about
  `shoes_boots` and `shoes_sandals` — items that no longer exist — so every real
  wheel fell through to `idHue(id)`. "Chrome Rims" came out whatever colour the
  string "shoes_chrome" happened to hash to.
* **Twenty-five wearables had no mesh at all** and fell through to a generic blob,
  so "Tiki Torch", "Golden Oar" and "Storm Harpoon" were the same object. All 87
  wearables have their own model now.

### And the rule that was killing sales

Equipping a kart shell **LOCKED your headgear and scarf** until you took it off.
You could not build a look. Building a look is the entire point — it is the
literal thing the research says people pay for. Gone. Wear the Mecha Frame with a
Paper Crown if you want: that combination **is** the expression, and someone will
love it.

## v23.19 — every item, and a podium that stands still

### No, I hadn't re-skinned all the items. Now I have.

Last time I did three and moved on. Here is what the other six actually were:

| Item | Was | Now |
|---|---|---|
| **Water balloon** | a sphere with a lump | a **teardrop** — fat where the water pools, a pinched neck, a hand-tied knot, a wet highlight, translucent, with the water **sloshing inside half a beat behind the skin** |
| **Beach ball** | a white sphere with four thin slivers | six real **gores** pole to pole, white caps where the panels converge, and a **valve stem** |
| **Water-pistol jet** | nine identical dabs in a line | a **coherent stream** at the muzzle with a bright core, breaking up into fatter, wobblier droplets, wrapped in fine mist |
| **The wave** | a half-cylinder with a torus balanced on top | a **breaking wave**: a green face, a **lip curling forward** over it, a lumpy white crest, spray flying off the top, foam washing along the sand |
| **Sprinkler** | a brown circle with a white cylinder | a real **garden sprinkler**: spike driven into the sand, squat green base, a **spinning arm with two angled nozzles**, water arcing out of them |
| **Sand pile** | a smooth cone with a wheel in it | a **collapsed sandcastle**: a slumped heap, the last turret still leaning, the **bucket that made it** tipped on its side, a **spade** stabbed in, and a kart wheel half-buried |
| **Geyser mine** | a torus and a translucent ball | a **buried whoopee cushion** — a rubber bladder half under the sand with its pinched spout sticking up, ringed by patted-down damp sand |
| **Rain cloud** | five grey spheres in a ring, box "rain" | a proper cartoon cloud: **flat dark bottom, lumpy sunlit top** (that silhouette is why you recognise one), taut **teardrops** falling, and a puddle forming |
| **Homing missile** | a burning **meteor** — a lump of space rock, on a beach, in a game about pool toys | a **rocket-powered rubber duck**. Beak, eyes, a firework gaffer-taped to its back, exhaust plume, smoke trail. You know instantly what's coming for you. |

There's a test that fails if any item visual drops back to three meshes or fewer.

**And the sprinkler's water never moved.** The droplets were built and then never
animated — they just hung in the air. Same for the water jet. Both fly properly now.

**The wave was falling over backwards.** A cylinder's top is +Y, and rotating about
Z by a POSITIVE angle swings it toward **-X** — away from you. The wave had leaned
the wrong way the whole time.

**The test that should have caught that couldn't.** It averaged the X of every
vertex above y=0.6 and below y=-0.6 and subtracted. On a *half*-cylinder every
vertex sits on one side, so the result was dominated by which ones crossed the
threshold — the measured "lean" **flipped sign between rotation.z of 0.3 and 0.5**.
It measured nothing. It reads the wall's axis now, which is unambiguous.

### The podium doesn't spin

It rotated the whole scene continuously, so for half of every revolution you were
looking at **the back of three karts**. A podium is a photograph, not a turntable.
It's framed once — a slight three-quarter angle, a touch above eye level, so the
karts have depth and the winner is where your eye lands first — and it stays framed.

## v23.18 — the second screenshot

### The wheels were sideways

Everything about the wheel was fighting itself. The **tyre** is a TorusGeometry
(spins about its own +Z). The **rim** was a cylinder rotated about Z, putting its
axle on **X** — ninety degrees out from the tyre it sits *inside*. The **spokes**
fanned about a third axis. And `animateCar` rolled the whole thing about a
**fourth**. On top of that, the steering animation drove the front wheels toward
`Math.PI/2` every frame — re-applying, forever, a compensating quarter-turn I'd
already removed from the build.

Measured in the live game: the front wheels were **89% aligned with the direction
of travel**. Fully broadside. One axle now (X), every part agrees, and there's a
test that fails if any wheel turns more than 20° off.

### The road was going through the floor
...
So the road hung in mid-air on every climb and was buried under the sand in every
dip. That's the street "disappearing". There are **embankments** now: skirts of
sand running from the road's edge down to the ground, so there is always terrain
holding the road up.

### The jump was a pit

Sandcastle's jump was a **35-metre gap** with a kick of 0.07. Run the ballistics:
at full race pace that kick carries you **eighteen metres**. There is no kick value
that clears thirty-five — even an absurd 0.45 only reaches thirty-one. **Every kart
fell in, every lap.**

I shrank it. Then I found the road **turns nineteen degrees across the gap** — and
a kart in mid-air flies in a straight line, so landing was geometrically impossible
regardless of the span. So I moved it to the straightest part of the circuit. There
the road *climbs* through the gap: you'd be jumping uphill into a wall.

The circuit's geometry cannot support a gap, and forcing one in is exactly why you
couldn't tell where you were meant to be going — there was nowhere to go. The road
is continuous now. The 8.9m crest stays, as a rise you carry speed over and a
descent you have to hold.

### The forest on the racing line

Roadside dressing was placed at `width/2 + 9` — nine metres past the road edge,
which is *inside* the drivable shoulder. And a circuit **loops back on itself**, so
a palm nine metres off one straight lands squarely in the middle of a different
one. **Thirty-four pieces were standing on the road on Sandcastle alone.** Every
piece is now checked against every sample of the whole track.

### You exploded for stopping

The sand burial fired on `offTrack && speed < 0.4 && offSince > 1.6`. But
`offTrack` is true the moment a wheel touches the **shoulder** — brake to line up a
corner with two wheels on the kerb and the sand ate you. Now you have to be
genuinely marooned: well out past the shoulder, stopped, and stay stopped for four
seconds. (The threshold also had to move *inside* the bumper rail — it was set
beyond it, so it could never have fired at all.)

### Real toys, not shapes

* **The water balloon** was a sphere with a lump. It's a **teardrop** now: fat and
  heavy at the bottom where the water pools, a pinched neck, a hand-tied knot, a
  wet highlight — and it's translucent, with the water **sloshing inside**, half a
  beat behind the skin.
* **The beach ball** was a white sphere with four thin slivers stuck on. It has six
  real **gores** running pole to pole, white caps where the panels converge, and a
  **valve stem**.
* **The item boxes** were gift-wrapped cubes — a Christmas present, on a beach.
  They're **sandcastle buckets**: tapered pail, rolled rim, wire handle, a turret
  with crenellations packed on top, and a little flag.

### A full UI revamp (the in-game HUD untouched, as asked)

The menus were a **dark interface** — near-black teal panels over a dusk gradient.
Measured on the real menu: **35% of every screen was that colour.** It looked like a
moody underwater game, not a bright summer kart racer.

It's daylight now. Sky, sea and hot sand as the page. White beach-towel cards.
Chunky, pressable beach-toy buttons with a real 3D lip. **Menu screens went from 35%
dark to 5%.**

And the CSS still contained a section literally headed **"WESTERN MOTIFS"** —
`.wood-panel`, `.leather-panel`, `.wanted-poster` — leftover chrome from the
gunslinger game, still styling every menu in a beach kart racer. Gone: boardwalk
planks, a striped beach towel, and a sun-bleached beach sign.

Every menu screen is verified for contrast: **zero unreadable elements.**

## v23.17 — the screenshot

Gustavo sent a screenshot. Nine problems, and every one of them was real.

**THE KART WAS SIDEWAYS.** The chassis is modelled along +Z (the natural way to lay
a vehicle out) but the engine's `heading = 0` means travelling along **+X**. So the
kart rendered a full 90° off — you could see the driver's shoulder where the nose
should be. **The old lathe body hid this completely**: a shape spun around an axis
has no front, so nothing looked wrong. The moment the kart had an actual nose, the
bug was glaring. The body is rotated once at build time now, and there's a test that
fires the nose-vs-travel dot product and fails below 0.9.

**FOUR OF THE SEVEN MODES COULD NOT BE SELECTED.** Every mode's `worldView()` and
`view()` read state that only exists *after* `init()` runs at match start — but the
LOBBY builds a view too (that's how the picker knows what you've chosen). Derby,
CTF, Sand Artist and Pearl Rush **threw**, which killed the entire broadcast, so the
client never learned the mode had changed and the button simply did nothing.

**TIME ATTACK WAS NEVER TIME ATTACK.** The client created the room with
`mode: "timetrial"` — a mode id that doesn't exist any more. `getMode()` returned
null, the engine fell back to a plain race, and the button just ran a solo race with
items off. The store had six more checks against the same dead string.

**KARTS CLIPPED THROUGH THE FLOOR ON RAMPS.** The floor check sat in the *middle* of
`stepCar` — a hundred lines before the position was even integrated. It was clamping
a height for a position the kart hadn't moved to yet. On a ramp, where you cover most
of a metre in a tick, that left karts **1.14m inside the road**, grounded, dead centre
of the track. It runs last now, against a freshly-resolved sample.

**MOUNTAINS ON THE ROAD.** The horizon dunes were placed on a fixed ring of radius
150–238 with radii up to 68 — while the track spans ±190. They landed *on the racing
line*. Every dune is now tested against every track sample before it's built.

**THE CROWD STANDS WERE SIDEWAYS.** The tiers are built along X and step back along
−Z, and the rotation put them side-on to the road — a row of platforms with the
spectators staring off into the desert. They face the race now.

**BOTS HAD NO DIFFICULTY.** `botTier` was **stored and then completely ignored** —
every bot from "rookie" to "ace" got the identical config. The hardest race in the
game was driven by exactly the same bots as the easiest. Four real tiers now:

| Tier | Best lap | A strong human beats them |
|---|---|---|
| Rookie | 139.5s | 6/6 |
| Pilot | 110.6s | 3/6 |
| Expert | 103.2s | **0/6** |
...
They differ in look-ahead, aggression, braking, a steering wander that makes a rookie
*look* like a rookie, and how fast they fire an item — **bots never used their items
at all before**, they'd carry a TSUNAMI around for three laps. Only the lower tiers
get any rubber-banding: beat an ace and you beat it clean.

**"DRAW!" IS GONE.** The countdown literally called `SpeechSynthesis` to *say the
word out loud* — a leftover from the gunslinger game this was forked from. Replaced
with a **"WELCOME TO <map name>"** card, and no countdown fighting the flythrough
(the F1 start lights already count you in).

**THE FLYTHROUGH IS SMOOTH.** It used to hard-`set()` the camera to a new spline
sample every frame with no interpolation — a series of teleports. It eases (smoothstep)
and lerps both the eye *and* the look-at now. It shows less of the lap, and that's the
right trade.

**ESC OPENS A MENU** — resume, sound on/off (there was no way to mute the game at
all), reset kart, the controls, and a two-step **SURRENDER**.
